...
export class FakeState {
  ...
  constructor(
    ...
    /**
     * Текст, который «распознан» на странице рабочего документа.
     *
     * Двойник не растеризует и не читает PDF, поэтому единственный способ
     * провести через настоящий конвейер настоящий текст документа — задать
     * его здесь. Это НЕ послабление двойника (S7: двойник не имеет права
     * быть мягче оригинала): форма ответа, состав архива, ссылки на кропы и
     * QA-инварианты остаются прежними, меняется только СОДЕРЖИМОЕ страницы —
     * ровно то, что в бою приносит OCR. Без этого сегментация проверялась бы
     * на строке «Синтетический распознанный текст блока …», то есть ни на чём.
     *
     * Индекс массива — `page_index` рабочего документа. Отсутствие значения
     * означает прежний синтетический текст.
     */
    readonly pageTexts: readonly (string | null | undefined)[] = [],
    /**
     * Один полностраничный TEXT-блок на страницу вместо раскладки детектора.
     *
     * Соответствует их профилю `full-page-text` (§5.3), законному для
     * однородных текстовых комплектов. Нужен там, где текст страницы обязан
     * совпасть с заданным ПОБУКВЕННО: при нескольких блоках страница склеена
     * из нескольких секций, и заголовок документа перестаёт быть первой
     * строкой.
     */
    readonly fullPageBlocks: boolean = false,
  ) {}
