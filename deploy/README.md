...
**Только через `deploy-id`, не через `docker compose … up` руками.** Compose,
запущенный из каталога `deploy` без `-p id`, берёт имя проекта из имени папки и
поднимает ВТОРОЙ стек `deploy-id-api-1` / `deploy-id-worker-1` / `deploy-id-web-1`
с образом `id-api:latest` — той сборки, что случайно оказалась под этим тегом.
Именно это случилось 06.09 (S58): дубликат под `restart: unless-stopped` пережил
три выкатки `-p id`, его воркер брал задачи из общей очереди кодом двухдневной
давности, а его API делил с боевым сетевой алиас `id-api` в сети `edge`. Отчёт
`deploy-id` с тех пор печатает строку `исполнители:` — см. §7.
...
### Разметка блоков идёт медленно (S50)

Замер боевой папки на 220 листов: проба разворота 13 с/лист, детекция 9,7 с/лист,
обе последовательно — 65 минут до первого вызова распознавания. Разбирать в этом
порядке, от самого крупного к мелкому:

1. **Настройка `detection.sheet_strategy`** (админка портала, не env).
   Умолчание `detect_all` запускает модель по каждой странице, и A4 при 300 DPI
   режется на 12 плиток — 12 вызовов сети на лист. Значение `sheet_aware`
   размечает лист A4 и мельче одним текстовым блоком БЕЗ модели и без рендера,
   а на крупном оставляет штамп и верхнюю надпись. Комплект ИД — почти целиком
   A4. Действует на НОВЫЕ ревизии разметки: после смены нажать «1. Выделить
   блоки» заново.
2. **Настройка `orientation.probe_enabled`.** Зонд разворота — вызов модели на
   каждый лист, и он же ставит задачи детекции: её темп ограничен его темпом.
   Сканы заведомо прямые — выключить (кнопки поворота на разметке остаются).
   Нужен — поднять `WORKER_CONCURRENCY_LLM` (потолок 4).
3. **Событие `queue_memory_pressure` в журнале воркера.** Сторож памяти запирает
   захват во ВСЕХ очередях сразу, и снаружи это неотличимо от «всё делается по
   одной задаче». Лечится парой `mem_limit` + `WORKER_RSS_SOFT_LIMIT_MB`, а не
   одним порогом.
4. **Квота процессора у `id-worker`** (`cpus`, `cpu_shares`). Детекция —
   счётная нагрузка; полтора ядра с половинным весом при конкуренции с соседями
   и есть разница «на порядок» с выделенной машиной.
5. **Настройка `detection.inference_mode`.** Манифест без `training_mode` даёт
   тайловый режим по умолчанию; модель, обученная на целой странице, при
   `whole_page` считает один кадр вместо двенадцати. Расхождение видно в
   журнале: `detect_local_mode_mismatch`.

#### Воркер на отдельной машине

Поддерживаемая топология, если разметка упирается в процессор общего VPS.
Воркер не делит с `id-api` ничего, кроме внешних служб: хранилище S3, БД и
proxy_llm он зовёт по сети сам, локальных путей между ними нет. Нужны те же
`/etc/id/id.env` и CA-сертификат, compose с одним сервисом `id-worker` (та же
команда `node apps/worker/dist/main.js`) и сетевой доступ к кластеру БД и
шлюзу. `WORKER_METRICS_PORT` задаётся в compose новой машины рядом с её
healthcheck — по той же причине, по которой он не живёт в env.

На старой машине воркер при этом остановить: два воркера на одну базу
допустимы по построению (аренда задач это и решает), но смысл выноса в том,
чтобы счётная нагрузка ушла с общего хоста. Compose второй машины обязан
передавать воркеру ту же `APP_RELEASE`, что и выкатка API, — иначе забор сборки
(ниже) его остановит, и правильно сделает. Выкатывать сначала хост с API, затем
хосты воркеров.

### Забор сборки (`deploy.release`, S58)

Два воркера на одну базу допустимы, но только ОДНОЙ сборки. Кто прав, решает
API: при старте он объявляет свою `APP_RELEASE` в `app_settings` под ключом
`deploy.release`. Воркер сверяет с объявленной свою метку при старте и каждые
15 секунд, а заодно сравнивает старшую миграцию базы (`schema_migrations`) со
старшей в каталоге `migrations/` своего образа. Исходы:

| Состояние     | Что значит                                                         | Захват задач             |
| ------------- | ------------------------------------------------------------------ | ------------------------ |
| `current`     | метки совпали                                                      | идёт                     |
| `unannounced` | API метку не объявлял (первая выкатка с забором, API лежит)        | идёт, warn               |
| `unverified`  | метки разошлись в первые 90 с после старта — API ещё поднимается   | удержан                  |
| `stale`       | разошлись позже, или база новее образа, или в production метки нет | удержан, выход с кодом 1 |
| `disabled`    | `APP_RELEASE` не задан вне production (dev, тесты)                 | идёт                     |

`stale` под `restart: unless-stopped` — это цикл перезапусков с
`worker_build_stale` в журнале (`own`, `announced`, `announced_at`, `reason`).
Это **намеренное красное состояние**, а не сбой: устаревший процесс перестал
быть невидимым конкурентом. Лечится выкаткой того же коммита на этот хост либо
снятием лишнего контейнера. Откат (`git checkout <SHA> && deploy-id`) забор не
ломает: API объявит откаченную метку, воркер несёт её же.

Метка теперь доезжает до api и worker через `environment` compose (раньше — только
до бандла id-web, а api/worker читали константу из `id.env`, и журнал ошибок
две недели подписывал все отказы сборкой от 21 августа). Строку `APP_RELEASE=`
в `/etc/id/id.env` оставить пустой или удалить. Каждая захваченная попытка
подписана сборкой в `job_runs.release`.

### Правила пропали из реестра: портал сам их вернёт

`rule_definitions` производен от каталога кода, поэтому недостающие определения
портал досевает при старте сам и пишет в журнал `rule_registry_backfilled` с
кодами. Строка означает: кто-то удалил из базы то, что кладут миграции. Портал
из-за этого больше не встаёт, но **снимок опубликованного набора он вернуть не
может** — тот неизменяем, и потерянные из него правила будут пропускаться как
«нет в снимке», пока не выйдет новая версия набора.

Обратное состояние — в базе есть код, которого нет в сборке, — остаётся отказом
старта (`rule_registry_mismatch`, `missingImplementations`). Это значит, что
процесс собран старее схемы: выкатите на него текущий образ. Удалять «лишние»
коды из базы, чтобы старый образ поднялся, НЕЛЬЗЯ: снимок набора ссылается на
них внешним ключом, и такая чистка ломает следующую выкатку — так дважды и
вышло 7 и 8 сентября (S58).

Проверить состояние:

```sql
select count(*) from rule_definitions;
select r.rule_code from ruleset_rules r join ruleset_versions v on v.id = r.ruleset_version_id
 where v.version = (select value #>> '{}' from app_settings where key = 'ruleset.active_version_id');
```

### Как найти чужой исполнитель

1. Отчёт `deploy-id`, строка `исполнители:` — все контейнеры хоста с командой
   api или worker вне проекта `id`.
2. Руками на хосте:

```bash
docker ps -a --format '{{.Names}}\t{{.Image}}\t{{.Label "com.docker.compose.project"}}\t{{.Command}}' \
  | grep -E 'apps/(worker|api)/dist'
docker compose ls                       # проектов с этим compose-файлом должен быть один: id
ps aux | grep -F 'apps/worker/dist/main.js' | grep -v grep   # процессы вне docker
```

3. По базе (`mcp_readonly`): чьи аренды и чьи попытки.

```sql
select locked_by, count(*) from jobs where status = 'running' group by 1;     -- host:pid
select release, outcome, count(*) from job_runs
 where started_at > now() - interval '1 day' group by 1, 2 order by 1, 2;    -- сборки за сутки
select value from app_settings where key = 'deploy.release';                 -- что объявил API
```

4. Снять лишний стек, не трогая `id`:
   `docker compose -p <проект> -f /opt/portals/id/deploy/docker-compose.prod.yml down`
   (без `--volumes`). `--remove-orphans` у `deploy-id` не помог бы: сироты живут
   в другом проекте compose, а не в `id`.

Другую машину этот раздел не видит — там смотрят то же самое; гарантия от
исполнителя чужой сборки одна на всех — забор в самом воркере.
