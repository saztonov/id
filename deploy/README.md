...
### Догонять `id.env` после спринтов

Файл ставится копией примера один раз, а `deploy/id.env.example` живёт в репозитории
и пополняется: ключи, добавленные спринтом, сами на сервер не приезжают. Код при
этом не падает — необязательные переменные молча уходят в умолчания, и выясняется
это через месяцы. Так на проде оказался выключен сторож памяти воркера (S41).

Поэтому `deploy-id` сверяет ИМЕНА ключей `/etc/id/id.env` с примером и печатает
недостающие строкой `env-ключи:` в отчёте (значения не читаются). Увидев её:
дописать ключи в `/etc/id/id.env` со значениями и комментариями из примера,
сохранив права `640 root:docker`, и повторить `deploy-id`. Деплой из-за этого не
прерывается: строка — предупреждение, а не отказ.

Обратное тоже верно: если ключ из примера убрали (как `WORKER_METRICS_PORT`,
переехавший в compose), лишняя строка в `/etc/id/id.env` ни на что не влияет —
`environment` в compose её перекрывает.
...
| compose | `WORKER_METRICS_PORT` + healthcheck   | Порт служебного `/metrics` задан в compose рядом с проверкой, которая к нему ходит; в `id.env` его не дублировать                       |
...
5. **Очередь `llm` молчит.** В журнале воркера ищите `queue_paused`, на
   `/metrics` — `job_queue_paused{queue="llm"}`: после трёх
...
### Маршруты проверяются отдельно от здоровья (S45)

`deploy-id` после подъёма дёргает несколько адресов `/api/v1` изнутри контейнера
и требует любой ответ, кроме 404: без сессии живой маршрут отвечает 401, и это
правильный ответ. Проверяется НАЛИЧИЕ маршрута, а не доступ.

Проверка появилась после того, как переименование маршрутов дошло до прода:
`/health/ready` живёт независимо от бизнес-маршрутов, поэтому выкатка с
полностью выпиленным `/api/v1` отчиталась бы «health: ok». В отчёте деплоя
строка `маршруты:`; при 404 деплой завершается ошибкой.

Список короткий (`/api/v1/folders`, `/api/v1/catalog/objects`,
`/api/v1/admin/jobs`) и живёт в самом скрипте: он отвечает на вопрос «поднялся
ли API как приложение», а полноту адресов проверяет тест
`apps/api/src/modules/navigation/client-routes.test.ts` — он сверяет все адреса
клиента с маршрутами собранного приложения.
