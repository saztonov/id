 ...
 *
 * Провайдер детекции учитывается здесь, а не в потребителях: легаси-путь RD WEB
 * (`layout.detect_pages`) правила форматов не знает и знать не будет, и если бы
 * стратегия применялась к нему наполовину — блоки по старому правилу, флаги по
