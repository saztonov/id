...
describe('normalizeDocNo', () => {
  ...
  describe('фолдинг гомоглифов', () => {
    ...
    it('аббревиатура органа, набранная транслитерацией, складывается с кириллической', () => {
      // Сертификат КНАУФ папки «ИД Мастер апрель 2026»: бланк печатает
      // «RU.СМИК.001.Н.00270», а опись передачи называет его в одних строках
      // так же, а в других — «RU.CMIK.001.H.00270». Буквы «С», «М», «К», «Н»
      // складывались и раньше, «И» — нет, и восемь строк описи получали подпись
      // «номер совпал не полностью» там, где сверять нечего.
      const cyrillic = normalizeDocNo('№ RU.СМИК.001.Н.00270');
      const latin = normalizeDocNo('№ RU.CMIK.001.H.00270');

      expect(cyrillic.normalized).not.toBe(latin.normalized);
      expect(cyrillic.folded).toBe(latin.folded);
    });

    it('транслитерация не склеивает номера, различные по существу', () => {
      // Чувствительность: пара «И ↔ I» обязана складывать букву, а не номер.
      expect(normalizeDocNo('RU.CMIK.001.H.00270').folded).not.toBe(
        normalizeDocNo('RU.CMIK.001.H.00271').folded,
      );
    });
