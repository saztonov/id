...
describe('markupPolicyFromSettings', () => {
  ...
  it('незнакомый провайдер получает легаси-политику, а не новое правило', () => {
    // `detection.provider` хранится строкой, и снятое значение может приехать
    // из восстановленной копии базы. Иначе блоки размечались бы по старому
    // правилу, а флаги считались бы по новому: анализ покрытия судил бы
    // разметку по чужому правилу.
    expect(
      markupPolicyFromSettings({
        provider: 'rdweb',
        sheetStrategy: 'sheet_aware',
