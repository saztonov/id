...
describe('REG.114 — организация в строке описи против документа', () => {
  ...
  it('организация не прочитана ни у одного документа вида — undetermined, а не молчание', () => {
    // Так «правило прошло» становилось неотличимо от «правило не смотрело»: на
    // папке «ИД Мастер апрель 2026» организация не извлечена ни у одной из
    // двенадцати исполнительных схем, REG.114 отвечало вердиктом без замечаний,
    // и четыре строки описи с чужим лицом остались неназванными.
    const graph = graphWithOrg('ООО "МАСТЕР"', null);
    expect(verdictOf('REG.114', graph)).toBe('undetermined');
    expect(messagesOf('REG.114', graph)[0]).toContain('не прочитана');
  });

  it('одиночный пропуск реквизита правило по-прежнему обходит молча', () => {
    // Чувствительность: у ОДНОГО документа реквизит не прочитан — это его беда,
    // о ней сообщают правила заполненности. Оговорка появляется только когда
    // реквизита нет у всех документов вида, то есть когда молчит портал.
    const bare = makeDocument({ docTypeCode: 'exec_scheme', title: 'Схема без реквизита' });
    const named = makeDocument({
      docTypeCode: 'exec_scheme',
      title: 'Схема с реквизитом',
      fields: [text('issuer', 'ООО «МАСТЕР»')],
    });
    const graph = makeGraph({
