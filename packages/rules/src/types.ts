...
export interface DocumentNode {
  ...
  /**
   * Комплект, которому документ принадлежит; `null` — вне комплектов (S44).
   *
   * Комплект — это акт освидетельствования со своим перечнем приложений,
   * приложениями и исполнительной схемой. Границу проводит сегментация; `null`
   * получают опись передачи, титульные листы и всё, что лежит в папке до
   * первого акта. Это законное состояние, а не потеря: приписать опись первому
   * попавшемуся акту значило бы соврать о составе.
   */
...
export interface RegistryRowNode {
   ...
   *
   * Правило «документ не назван реестром» обязано считать их названными: они
   * реестром упомянуты, и объявить их лишними значило бы обвинить комплект
   * дважды за одно. Пересчитывать похожесть внутри правила нельзя — вторая
   * реализация лестницы разойдётся с первой молча.
   */
...
export interface CheckGraph {
  ...
  /**
   * Сколько листов ревизии портал НЕ разобрал.
   *
   * Лист попадает сюда, если с него ничего не прочитано (и это не
   * подтверждённый пустой оборот) либо прочитанное не отнесено ни к одному
   * документу. Число нужно правилам полноты: вывод «документа нет в комплекте»
   * опирается на то, что комплект разобран весь, и при ненулевом значении
   * такой вывод сделать нельзя — это `undetermined`, а не дефект помягче.
   */
  readonly coverageGaps: number;
  /**
   * Те же пробелы, но с адресом: сколько их у каждого комплекта (S50).
   *
   * Без адреса один неразобранный лист гасил выводы о полноте по ВСЕЙ папке:
   * срез комплекта получал общее число, и двенадцать комплектов боевой папки
   * отвечали «не проверено» из-за листа, лежащего в одном из них. Ключ — тот
   * же идентификатор комплекта, что у документов; листы вне комплектов живут
   * в `coverageGapsOutside`.
   *
   * Считает тот, кто знает ПОРЯДОК листов: лист без документа принадлежит
   * тому комплекту, чьи страницы его окружают. Правилам эта арифметика не
   * видна — они получают уже своё число.
   */
  readonly coverageGapsByComplect: Readonly<Record<string, number>>;
  /** Неразобранные листы, не попавшие ни в один комплект. */
  readonly coverageGapsOutside: number;
}
...
export type RuleKind =
   ...
   *
   * «Значение реквизита расходится с текстом документа» — это утверждение о
   * том, что портал прочитал неверно, и подрядчику с ним делать нечего:
   * исправлять надо не документ. На боевой папке таких предупреждений 61, и они
   * лежали вперемешку с настоящими дефектами, где их и принимали за дефекты.
   *
   * Вид, а не признак у замечания: он объявлен в каталоге, попадает в
   * `rule_definitions.kind` и потому читается одинаково отчётом, счётчиками и
   * администрированием. Признак у замечания пришлось бы выводить по коду
   * правила в трёх местах, и они разошлись бы при первом же новом правиле.
   */
  | 'extraction_quality';
...
export interface RuleExecution {
   ...
   *
   * Разрез журнала, названный долгом ещё в ADR-0018 с оговоркой «для комплекта
   * одной работы неоднозначности нет». На папке с двенадцатью актами её нет уже
   * не по построению: без разреза «правило пройдено» смешивало бы двенадцать
   * разных ответов в один.
   */
  readonly complectId?: string | null;
}
...
export interface RuleRunCounts {
  /** Сколько правил вообще есть в каталоге прогона. */
  readonly rulesTotal: number;
  /**
   * Сколько ИСПОЛНЕНИЙ состоялось — не сколько правил (S44).
   *
   * После нарезки папки на комплекты одно правило исполняется по разу на
   * комплект, и `executed` законно превышает `rulesTotal`. Читать его как «из
   * скольких правил» нельзя; `passed`, `failed`, `undetermined` и
   * `notApplicable` считаются в тех же единицах.
   */
