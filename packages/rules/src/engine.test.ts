...
describe('reconcileRuleRegistry', () => {
  ...
  /**
   * Третий список — снятые правила (S30, S59). Их строки остаются в БД
   * навсегда (внешний ключ замечаний и снимков), а реализации нет — и это
   * решение, а не расхождение. Но послабление действует в ОДНУ сторону.
   */
  it('снятый код в БД без реализации — не расхождение', () => {
    expect(() => {
      reconcileRuleRegistry(['A.1', 'B.2', 'R.9'], specs, ['R.9']);
    }).not.toThrow();
  });

  it('снятый код, которого в БД нет, сверке не мешает', () => {
    // Снятие описывает, чего от БД НЕ требовать, а не что в ней обязано быть.
    expect(() => {
      reconcileRuleRegistry(['A.1', 'B.2'], specs, ['R.9']);
    }).not.toThrow();
  });

  it('снятие не оправдывает обратное направление: реализация без записи — отказ', () => {
    // Код, названный снятым, но всё ещё в каталоге реализаций, — это правило,
    // которое движок исполняет, а профиль включить не может.
    try {
      reconcileRuleRegistry(['A.1'], specs, ['B.2']);
      expect.unreachable('сверка обязана отказать');
    } catch (error) {
      expect(error).toBeInstanceOf(RuleRegistryError);
      expect((error as RuleRegistryError).missingDefinitions).toEqual(['B.2']);
      expect((error as RuleRegistryError).missingImplementations).toEqual([]);
    }
  });
