 ...
 */
import type { FindingSeverity, RuleFinding, RuleFn, RuleResult, RuleVerdict } from './types.js';

/**
 * Реализация снятого с исполнения правила.
 *
 * Спек снятого правила обязан остаться в коде: его строки напечатаны в
 * применённых миграциях сида и снимков (0017…0081), и тест дрейфа сверяет их
 * контрольные суммы с выводом генераторов. Но исполнять его нельзя — движок его
 * и не исполняет, потому что кода нет в `RULE_CATALOG`. Заглушка нужна ровно
 * затем, чтобы у спека не осталось живого тела: реализация, которую никто не
 * зовёт, разошлась бы с движком молча, а вернуть её в каталог было бы делом
 * одной строки без единого красного теста.
 */
export const retiredRule: RuleFn = () => notApplicable('правило снято с исполнения');
