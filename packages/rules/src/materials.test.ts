 ...
 * напрямую, она делает хуже: `DATE.312` (единственный потребитель партий после
 * снятия `MAT.*` и `DATE.372` в S59) перестаёт находить дефекты, оставаясь
 * зелёным, потому что проверять оказывается нечего. Поэтому тесты смотрят на
 * состав узлов, а не на «функция вернула массив».
 */
