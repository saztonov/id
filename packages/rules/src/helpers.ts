...
/**
 * Организационно-правовая форма: полное написание и его аббревиатура.
 *
 * Форма приводится к аббревиатуре, а НЕ выбрасывается. Разница существенна:
 * выброс склеил бы «ООО „Ромашка“» с «АО „Ромашка“» — разными юридическими
 * лицами, — а приведение оставляет их различимыми и при этом равняет
 * «Общество с ограниченной ответственностью „КНАУФ ГИПС“» из сертификата с
 * «ООО „КНАУФ ГИПС“» из паспорта качества.
 *
 * Ровно на этой паре `MAT.111` дважды объявляло на папке «ИД Мастер апрель
 * 2026» ошибку «изготовитель партии не покрыт ни одним сертификатом», хотя
 * сертификат на этого изготовителя в комплекте лежит.
 *
 * ## Почему замена идёт ДО фолдинга
 *
 * Прежний список форм стоял после `foldHomoglyphs` и не срабатывал никогда:
 * фолдинг уже превратил кириллические «О», «А», «К», «Н» в латинские, и
 * кириллический шаблон `ООО` не совпадал с латинским `OOO` в строке. Плюс `\b`
 * в JS опирается на ASCII и с кириллицей границы слова не даёт. Оба дефекта
 * снимаются одним переносом: форма распознаётся на исходном тексте, где буквы
 * ещё кириллические, а границы заданы явным перечнем разделителей.
 */
const LEGAL_FORMS: readonly (readonly [RegExp, string])[] = [
  [/обществ\S*\s+с\s+ограниченной\s+ответственност\S*/giu, 'ООО'],
  [/публичн\S*\s+акционерн\S*\s+обществ\S*/giu, 'ПАО'],
  [/непубличн\S*\s+акционерн\S*\s+обществ\S*/giu, 'НАО'],
  [/закрыт\S*\s+акционерн\S*\s+обществ\S*/giu, 'ЗАО'],
  [/открыт\S*\s+акционерн\S*\s+обществ\S*/giu, 'ОАО'],
  [/акционерн\S*\s+обществ\S*/giu, 'АО'],
  [/индивидуальн\S*\s+предпринимател\S*/giu, 'ИП'],
];
...
export function normalizeOrgName(value: string): string {
  let expanded = value;
  for (const [pattern, abbreviation] of LEGAL_FORMS) {
    expanded = expanded.replace(pattern, abbreviation);
  }

  return foldHomoglyphs(expanded)
    .replace(/[«»"'`]/gu, '')
    .replace(/[^\p{L}\p{N}]+/gu, '')
    .trim();
}
