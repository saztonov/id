...
export interface RunRulesOptions {
   ...
   *
   * `null` — «ограничений нет»: исполняются все правила снимка. Пустой массив
   * движок по-прежнему читает буквально — ни одного правила, — но с S58 такой
   * список сюда не попадает: пустой список профиля вызывающий переводит в
   * `null`, а снятые объектом правила — в «снимок минус снятые». Буквальное
   * чтение оставлено движку намеренно: оно проверяемо и не зависит от того, как
   * профиль трактует пустоту.
   */
