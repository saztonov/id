...
export const BUILTIN_RULESETS: readonly BuiltinRuleset[] = [
  ...
  },
  {
    ...
    //
    // Состав ЗАФИКСИРОВАН по своей миграции, а не взят «весь каталог» (S57).
    // Пока последний набор объявлял себя каталогом целиком, каждое новое
    // правило молча переписывало ПРИМЕНЁННУЮ миграцию — раннер объявил бы её
    // изменённой и отказался применять. Набор описывает состояние на момент
    // публикации, и это состояние обязано быть неподвижным.
    migration: '0075_builtin_ruleset_3',
    version: 'builtin-3',
    bootstrap: false,
    specs: seededBefore('0075_builtin_ruleset_3'),
  },
  {
    // Остальные виды расхождений строки описи (S57): REG.115–117. Набор
    // последний, и только он вправе брать каталог целиком: следующее правило
    // заведёт следующую версию, а этот к тому времени будет зафиксирован по
  ...
  },
  {
    ...
    //
    // Состав ЗАФИКСИРОВАН по своей миграции (S59): набор перестал быть
    // последним. `seededBefore('0081…')` — партии 0017, 0031, 0034, 0067,
    // 0074, 0077, ровно то множество, что этот файл печатал как «весь каталог».
    migration: '0081_builtin_ruleset_5',
    version: 'builtin-5',
    bootstrap: false,
    seedsDefinitions: true,
    specs: seededBefore('0081_builtin_ruleset_5'),
  },
  {
    /**
     * Минимальный набор проверок (S59, ADR-0029).
     *
     * Снимок — только ДЕЙСТВУЮЩИЕ правила (`RULE_CATALOG`), а не каталог со
     * снятыми. Строка `is_enabled = true` у правила, которого движок не
     * исполняет, — ложь о том, что проверял прогон; строка с `false` — балласт,
     * который администратор включить не может (`GET /admin/rules` снятые не
     * отдаёт). Внешние ключи снимка разрешимы: определения снятых правил уже в
     * БД, а досев кладёт действующие — в том числе два новых из партии 0082.
     */
    migration: '0083_builtin_ruleset_6',
    version: 'builtin-6',
    bootstrap: false,
    seedsDefinitions: true,
    specs: RULE_CATALOG,
