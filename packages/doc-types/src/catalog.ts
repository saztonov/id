...
export const DOC_TYPES = [
  ...
  {
    // Код остаётся `aosr_responsible_structures`, хотя форма переименована
    // (АООК → АОСК, приказ Минстроя): на префиксе `aosr` держатся `ACT_TYPES`,
    // `isAnalysisAnchor` и группировка комплектов, а смена кода потянула бы
    // за собой внешние ключи и применённые миграции.
    code: 'aosr_responsible_structures',
    name: 'Акт освидетельствования строительных конструкций (АОСК, бывш. АООК)',
    shortName: 'АОСК',
    group: 'acts',
    kind: 'primary',
    hasAnnexes: false,
    ...
    matchHints: {
      // Оба титула: старая форма «ответственных конструкций» встречается в
      // комплектах, составленных до переименования, и снять её нельзя.
      // Третий якорь — под рендер VLM, склеивающий «АКТ» и вторую строку
      // титула в одну (та же находка, что у `aosr` выше).
      anchors: [
        'освидетельствовани[яе]\\s+ответственных\\s+конструкций',
        'освидетельствовани[яе]\\s+строительных\\s+конструкций',
        'АКТ\\s+освидетельствовани[яе]\\s+(ответственных|строительных)\\s+конструкций',
      ],
  ...
  },
  {
    ...
    observedInCorpus: false,
    matchHints: {
      // Две записи титула, как у `aosr`: рендер RD WEB разбивает его на «АКТ»
      // и «входного контроля», рендер VLM склеивает в одну строку, а якорь
      // обязан совпадать с началом строки.
      anchors: ['АКТ\\s+входного\\s+контроля', 'входного\\s+контроля'],
      // Журнал входного контроля (S59) озаглавлен теми же словами. В рендере
      // с титулом в две строки вторая строка «входного контроля» неотличима
      // от акта, и без отрицательного якоря журнал становился актом.
      negativeAnchors: ['ЖУРНАЛ(?!\\p{L})'],
  ...
  {
    // Исполнительный чертёж — из таблицы заказчика (S59). Отдельный вид, а
    // не якорь к схеме: у чертежа собственный титул и шифр, и в реестре
    // приложений он называется своим именем. Якоря схемы его не ловят:
    // «исполнительн… схем» и «исполнительн… съемк» на слове «чертёж» не
    // срабатывают, а печатную подсказку бланка АОСР («исполнительные схемы и
    // чертежи») отсекают те же отрицательные якоря, что у схемы.
    code: 'exec_drawing',
    name: 'Исполнительный чертёж',
    shortName: 'Исп. чертёж',
    group: 'exec_schemes',
    kind: 'evidence',
    hasAnnexes: false,
    isFallback: false,
    observedInCorpus: false,
    matchHints: {
      // `[её]`: нормализация строки «ё» не сводит, а на бланках встречаются
      // оба написания.
      anchors: ['исполнительн\\S*\\s+черт[её]ж'],
      negativeAnchors: [
        '(\\d+\\.\\s*)?Предъявлены\\s+документы,?\\s+подтверждающие',
        'Реестр\\s+приложений',
      ],
      bodyHints: ['УСЛОВНЫЕ\\s+ОБОЗНАЧЕНИЯ', 'в/о\\s+\\S+', 'на\\s+отм\\.'],
    },
    fieldSchema: [
      field('scheme_title', 'Наименование чертежа', 'text', true, 'llm'),
      field('scheme_number', 'Шифр чертежа', 'text', true, 'rule'),
      field('sheet_no', 'Номер листа', 'text', false, 'rule'),
      field('executor', 'Организация, составившая чертёж', 'text', true, 'llm'),
    ],
    sortOrder: 105,
  },
  {
  ...
  {
    // Журнал входного контроля — из таблицы заказчика (S59). Титул совпадает
    // с актом входного контроля во всём, кроме первого слова, поэтому виды
    // разводятся зеркальными отрицательными якорями: у акта — «ЖУРНАЛ», у
    // журнала — «АКТ». Второй якорь нужен рендеру с титулом в две строки
    // («ЖУРНАЛ» / «входного контроля»), где первая строка отдельно, — и без
    // отрицательного якоря он один притянул бы любой акт входного контроля.
    code: 'incoming_control_log',
    name: 'Журнал входного контроля',
    shortName: 'Журнал ВК',
    group: 'registries_logs',
    kind: 'evidence',
    hasAnnexes: false,
    ...
    observedInCorpus: false,
    matchHints: {
      anchors: ['ЖУРНАЛ\\s+входного\\s+контроля', 'входного\\s+контроля'],
      // `(?!\p{L})` — слово целиком: «АКТИВ…» актом не является.
      negativeAnchors: ['АКТ(?!\\p{L})'],
      bodyHints: [
        'поступил[аи]\\s+на\\s+объект',
        'сопроводительн(ая|ые)\\s+документаци',
        'результат\\S*\\s+входного\\s+контроля',
      ],
    },
    fieldSchema: [
  ...
  },
  {
    ...
    observedInCorpus: true,
    matchHints: {
      // «Учётный лист» — то, как журнал авторского надзора приходит в комплект
      // на деле: подшивается не журнал, а его лист, и озаглавлен он «УЧЕТНЫЙ
      // ЛИСТ № 118». Реестр приложений называет его «ЖАН №2, учетные листы
      // №118» — то есть номер листа и есть номер, по которому сверяется
      // строка. Без якоря такие листы оставались служебными страницами и не
      // относились ни к одному документу: в папке их пять из двенадцати
      // неотнесённых.
      anchors: ['ЖУРНАЛ\\s+авторского\\s+надзора', 'УЧ[ЕЁ]ТНЫЙ\\s+ЛИСТ'],
      bodyHints: [
        'замечани[яе]\\s+авторского\\s+надзора',
        'отметка\\s+об\\s+устранении',
        'авторск\\S*\\s+надзор',
        'Выявленные\\s+отступления',
      ],
    },
    fieldSchema: [
  ...
  {
    code: 'state_registration_certificate',
    ...
    observedInCorpus: true,
    matchHints: {
      // Заголовок бланка ТС печатается в две строки: «СВИДЕТЕЛЬСТВО» и ниже —
      // «о государственной регистрации». Склейка разорванного заголовка сюда
      // не дотягивается: она собирает только строки, набранные ПРОПИСНЫМИ, а
      // вторая строка строчная. Поэтому якорей два — на цельную запись и на ту
      // самую вторую строку. Ложных срабатываний она не даёт: строка реестра
      // начинается со слова «Свидетельство», а не с «о», и хвост у неё длиннее
      // служебного.
      anchors: [
        'СВИДЕТЕЛЬСТВО\\s+о\\s+государственной\\s+регистрации',
        'о\\s+государственной\\s+регистрации',
      ],
      // Приложение к СГР — лист того же документа, а не новый документ: он
      // присоединяется ролью annex_continuation по номеру родителя.
      negativeAnchors: ['ПРИЛОЖЕНИЕ', 'К\\s+СВИДЕТЕЛЬСТВУ\\s+о\\s+государственной\\s+регистрации'],
      bodyHints: [
        'ТАМОЖЕННЫЙ\\s+СОЮЗ',
        'ГЛАВНЫЙ\\s+ГОСУДАРСТВЕННЫЙ\\s+САНИТАРНЫЙ\\s+ВРАЧ',
        'Единым\\s+санитарно',
        'настоящим\\s+свидетельством\\s+удостоверяется',
      ],
    },
    fieldSchema: [
      field('application_area', 'Область применения продукции', 'text', false, 'llm'),
      field('registration_authority', 'Орган, выдавший свидетельство', 'text', false, 'llm'),
    ],
    sortOrder: 242,
  },
  {
  ...
  },
  {
    ...
    isFallback: false,
    observedInCorpus: true,
    // Таблица заказчика S59: вид проверкам не нужен. `observedInCorpus`
    // остаётся — это история о корпусе, а не о составе комплекта.
    retired: true,
    matchHints: {
  ...
  },
  {
    ...
    // Имя по таблице заказчика (S59): бланки разных заводов озаглавлены
    // одинаково — «Сертификат качества», без слов об изготовителе.
  ...
  {
    /**
     * Общий документ о качестве: ни смесь, ни штучное изделие (S53).
     *
     * Форма встречается в отделке: «Документ о качестве» на шпаклёвку готовую
     * пастообразную печатается заводским бланком БЕЗ номера рядом с
     * заголовком, и короткий якорь `mix_quality_doc` («ДОКУМЕНТ О КАЧЕСТВЕ №»)
     * на нём не срабатывает. Лист оставался без вида, роль «копия верна»
     * забирала его в соседний документ, и шесть строк описи «Документ о
     * качестве № 99-32» в папке «ИД Мастер апрель 2026» не находили ничего.
     *
     * Отдельным видом, а не якорем к смеси: схема полей смеси (класс
     * прочности, удобоукладываемость, сохраняемость, время отгрузки) к
     * пастообразной шпаклёвке неприменима, а правила смеси на ней дали бы
     * ложные замечания о марке и сохраняемости.
     */
    code: 'quality_doc',
    name: 'Документ о качестве',
    shortName: 'Документ о качестве',
    group: 'quality_docs',
    kind: 'evidence',
    hasAnnexes: false,
    isFallback: false,
    observedInCorpus: true,
    /**
     * Приоритет ниже смеси и изделий: там, где сработали их якоря, вид
     * известен точнее, и общая форма спорить с ними не должна.
     */
    priority: -1,
    matchHints: {
      // Без «№» и без предмета: формы смеси и изделий разбирают свои виды, и
      // отрицательные якоря отдают их им.
      anchors: ['ДОКУМЕНТ\\s+О\\s+КАЧЕСТВЕ'],
      negativeAnchors: [
        // Краткая форма с номером — каноническая у смеси: там номер партии
        // стоит прямо в заголовке, и вид известен точнее.
        'ДОКУМЕНТ\\s+О\\s+КАЧЕСТВЕ\\s*№',
        'ДОКУМЕНТ\\s+О\\s+КАЧЕСТВЕ\\s+(БЕТОННОЙ\\s+СМЕСИ|РАСТВОРА|СМЕСИ)',
        'ДОКУМЕНТ\\s+О\\s+КАЧЕСТВЕ\\s+\\S+\\s+ЗАДАННОГО\\s+КАЧЕСТВА',
        'ДОКУМЕНТ\\s+О\\s+КАЧЕСТВЕ.{0,40}на\\s+(камн|блок|плит|кирпич|издели|сва|панел)',
      ],
      bodyHints: ['Результаты\\s+испытаний', '№\\s*партии', 'Дата\\s+изготовления'],
    },
    fieldSchema: [
      field('product_name', 'Наименование продукции', 'text', true, 'llm'),
      field('batch_number', 'Номер партии', 'text', true, 'rule'),
      field('manufactured_at', 'Дата изготовления', 'date', true, 'rule'),
      field('nd_reference', 'ГОСТ или ТУ на продукцию', 'text', false, 'rule'),
    ],
    sortOrder: 296,
  },
  {
    ...
    isFallback: false,
    observedInCorpus: true,
    // Таблица заказчика S59: вид проверкам не нужен.
    retired: true,
    matchHints: {
  ...
  },
  {
    ...
    matchHints: {
      ...
      // Третий и четвёртый якоря — по первой строке самого письма. В корпусе
      // орган по сертификации не озаглавливает документ вовсе: после шапки с
      // адресом сразу идёт обращение. Без них письмо «Эксперт-С» на трёх
      // листах опознавалось продолжением соседнего паспорта качества, и строка
      // реестра «Отказное письмо» не находила своего документа.
      anchors: [
        'отказн[а-яё]+\\s+письм',
        'Информационное\\s+письмо',
        'На\\s+Ваш\\s+запрос',
        'в\\s+порядке\\s+информации\\s+сообщаем',
      ],
      bodyHints: [
  ...
  },
  {
    ...
    isFallback: false,
    observedInCorpus: true,
    // Таблица заказчика S59: вид проверкам не нужен. Правило CONCL.660
    // ссылается на код внешним ключом, поэтому запись остаётся.
    retired: true,
    matchHints: {
      ...
      negativeAnchors: [
        ...
        // Второй лист санитарно-эпидемиологического заключения: на нём
        // «ЗАКЛЮЧЕНИЕ» — подзаголовок вывода, а не титул документа. Отличает
        // его начало листа — гигиеническая таблица, попадающая в зону
        // заголовка. Без признака каждое санзаключение корпуса разрывалось
        // надвое: второй лист объявлялся техническим заключением с тем же
        // номером, и строка реестра «Экспертное заключение» становилась
        // неоднозначной, а «Приложение к экспертному заключению» — потерянным.
        'Гигиеническ\\S*\\s+характеристик',
        'санитарно[\\s-]эпидемиологическ\\S*\\s+экспертиз',
      ],
      bodyHints: [
...
// Через интерфейс, а не через литеральный тип кортежа: у видов без `retired`
// этого свойства в выведенном типе нет, и обращение к нему — ошибка типов.
const ALL_DOC_TYPES: readonly DocTypeDefinition[] = DOC_TYPES;

/**
 * Виды, которые портал распознаёт и предлагает.
 *
 * Полный `DOC_TYPES` нужен сиду, внешним ключам и чтению исторических
 * документов; всё, что решает, ЧТО портал видит в новых страницах и что
 * показывает в списках, обязано читать этот список — иначе снятый вид вернулся
 * бы в работу через чёрный ход (см. `DocTypeDefinition.retired`).
 */
export const ACTIVE_DOC_TYPES: readonly DocTypeDefinition[] = ALL_DOC_TYPES.filter(
  (type) => type.retired !== true,
);

/** Коды снятых видов (S59): их отклоняет профиль раздела и не отдаёт список видов. */
export const RETIRED_DOC_TYPE_CODES: readonly string[] = ALL_DOC_TYPES.filter(
  (type) => type.retired === true,
).map((type) => type.code);
