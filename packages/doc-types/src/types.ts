...
export interface DocTypeDefinition {
  ...
  /**
   * Вид снят с использования (S59).
   *
   * Остаётся в `DOC_TYPES`, но не распознаётся якорями, не предлагается ни
   * модели, ни человеку в списках, а в профиле раздела считается ошибкой.
   * Удалить запись нельзя, и причина механическая, а не историческая: на код
   * ссылаются внешние ключи (`logical_documents`, `page_classifications`,
   * `rule_definitions`), применённые сид-миграции защищены контрольной суммой,
   * а тест дрейфа сида требует, чтобы `doc_types` совпадала с каталогом ровно.
   * Тот же приём, что у снятых правил — `RETIRED_RULES` в `packages/rules`.
   */
