...
const EXPECTED_TABLES: readonly string[] = [
  ...
  'doc_type_candidates',
  'page_roles',
  // §3 после S44: объект → раздел → ПАПКА → комплект. Уровни «комплект работы»
  // и «ревизия поставки» схлопнуты в папку (0059), а реестры передачи со сверкой
  // описи сняты вместе с согласованием (0058): опись приходит страницами внутри
  // того же файла и проверяется как обычный документ.
  'folders',
  'complects',
  'stored_blobs',
  'source_files',
  ...
  'layout_block_points',
  'rd_run_documents',
  // Снимок исполнительной документации в RD WEB (0069, контракт document-sync
  // v1): логический документ, реестр устойчивых внешних идентификаторов блоков
  // и журнал отправок. Реестр — то, чем повторное распознавание остаётся
  // дешёвым: неизменившийся блок сохраняет свой `external_block_id`.
  'rd_exec_documents',
  'rd_exec_blocks',
  'rd_exec_syncs',
  ...
  // Разворот содержимого страницы (0052): поправка к скану, легшему боком при
  // нулевом /Rotate. Отдельной таблицей, потому что `source_pages` заперта
  // триггером неизменяемости уже в submitted — то есть ровно тогда, когда
  // инженер размечает и разворачивает.
  'page_orientations',
  'document_relations',
  'field_values',
  'registry_rows',
  ...
  // Метки инженера на строках перечня (0079): чем измеряется качество сверки,
  // которую судит модель. Живут вместе со строкой и переживают прогоны правил:
  // «этой строке отвечает вот этот документ» — факт о бумаге, а не о прогоне.
  'registry_row_labels',
  'materials',
  'batches',
  'material_documents',
