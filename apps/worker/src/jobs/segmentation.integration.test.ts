 ...
 *
 * Утверждение прямое, а не инвариантное, и это правка S37. Прежняя редакция
 * вычисляла ожидание из тех же `field_values`, которые и проверяла, — то есть
 * сравнивала прогон с самим собой и оставалась зелёной, пока дата акта
 * НЕ извлекалась вовсе. Ровно это и происходило: титул бланка занимает три
 * строки, «от» стоит под словом «АКТ», и обе прежние ступени `act_date` мимо
 * него проходили.
 *
 * Комплект заведён без месяца, поэтому «2026-02-01» здесь могло появиться
 * только от прогона по акту фикстуры («№ 01-ТЕСТ от «27» февраля 2026»).
 */
describe('месяц комплекта', () => {
  it('равен месяцу самой ранней даты акта', async () => {
    ...
    // Без первой утверждение о втором не отличало бы «прочитали 27 февраля» от
    // «не прочитали ничего и подставили».
    expect(actDates[0]?.value_date ?? null).toBe('2026-02-27');
    expect(work[0]?.period ?? null).toBe('2026-02-01');
  });
