 ...
 *
 * Собственный номер листа напечатан ВНЕ прямоугольника штампа: в самом штампе
 * стоит «Обозначение» проекта, общее у всех листов раздела, и без номера каждая
 * исполнительная схема оказывалась бы «нет в комплекте» при сверке с реестром
 * приложений. Другого источника у номера нет: на пути RD WEB `stamp.sheetCode`
 * всегда `null`, а на пути VLM он добирается дозапросом кропа, которого у RD
 * WEB не существует.
 *
 * Замер на двенадцати крупных листах эталонного комплекта: номер напечатан
 * ЗАГОЛОВКОМ ВВЕРХУ листа, а не рядом со штампом. Прежняя околоштамповая зона
 * его не брала — она давала семь верных номеров, пять ложных и девятнадцать
 * лишних блоков. Один самый верхний текстовый кандидат берёт те же семь
 * носителей двенадцатью блоками. Оставить один штамп нельзя: тогда верных
 * номеров становится ноль, а ложных двенадцать — в текст страницы побеждает
 * кадастровый номер участка из поля «Объект» штампа.
 *
 * Синтетический прямоугольник НЕ рисуется: берётся настоящий кандидат
 * детектора. Нет кандидатов — нет и номера, и это видно по флагу внимания.
 *
...
export function selectLargeSheetBlocks(
  ...
  // замере не добавил ни одного носителя номера, зато удваивал счёт блоков.
  // Тай-брейк по левой границе нужен не для качества, а для повторяемости:
  // порядок кандидатов детектора не обещан, и без него один и тот же лист мог
  // бы дать разные наборы блоков.
  let topmost: Candidate | null = null;
  for (const candidate of candidates) {
    if (candidate.blockType !== 'text') continue;
    if (topmost === null) {
      topmost = candidate;
      continue;
    }
    const dy = candidate.coordsNorm[1] - topmost.coordsNorm[1];
    if (dy < 0 || (dy === 0 && candidate.coordsNorm[0] < topmost.coordsNorm[0])) {
      topmost = candidate;
    }
  }
...
export function createLocalDetectionHandler(
  ...
  return async (ctx: JobContext<'layout.detect_local'>) => {
    ...
    /**
     * Анализ покрытия ставится с ОТСРОЧКОЙ, а не через секунду (S50).
     *
     * Тот же dedupeKey-паттерн, что у легаси-пути (`markup.ts`): анализ считает
     * флаги по всему комплекту и идемпотентен, лишний запуск безвреден
     * логически — но не по цене. Ключ освобождается, как только задача
     * выполнена, поэтому на папке из 220 страниц покрытие пересчитывалось 215
     * раз, по три секунды каждый, и делало это в очереди `io` ПАРАЛЛЕЛЬНО
     * детекции — то есть отбирало у неё процессор того же контейнера.
     *
     * Полминуты выбраны так, чтобы окно перекрывало обработку соседних страниц
     * (лист размечается единицы секунд): пока детекция идёт, задача стоит в
     * очереди и склеивает соседей ключом; последняя страница даёт последний
     * пересчёт, и он же оказывается итоговым. Быстрее пересчитывать незачем —
     * флаги покрытия читает человек на экране разметки, а не конвейер.
     */
    await ctx.enqueue({
