...
describe('цепочка «Разметить файл» доводит комплект до координат', () => {
  ...
  /**
   * Проверяется ПРАВКА, а не удаление (S24).
   *
   * Гейт заморозки держится на `blocks_hash`: прогон распознавания сверяется с
   * ним, и блок, изменившийся после заморозки, расходится с хэшем, по которому
   * шёл OCR. Именно это и обязано быть невозможным.
   *
   * Удаление блоков ЧЕРНОВОЙ ревизии с миграции 0035 разрешено: пока ревизия не
   * подана, доказывать нечего, а безусловный запрет не давал подрядчику убрать
   * ошибочный файл из собственного черновика — «Распознать» замораживает
