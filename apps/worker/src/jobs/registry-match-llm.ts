...
function prefilterNote(row: PartitionRow): string {
  if (row.matchState === 'matched' && row.matchScore === 1) {
    return `номер совпал посимвольно с документом ${row.matchedDocumentId ?? '—'}; проверь только описание`;
  }
  if (row.matchState === 'matched') {
    return `номер сошёлся после свёртки написания с документом ${row.matchedDocumentId ?? '—'}`;
  }
