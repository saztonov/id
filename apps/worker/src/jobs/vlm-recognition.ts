  ...
  return withVlmRunTermination(deps, async (ctx: JobContext<'vlm.start_recognition'>) => {
    ...
    for (const page of pages) {
      await ctx.enqueue({
        ...
        /**
         * Приоритет убывает с номером страницы (S41).
         *
         ...
         * второй не двигался вовсе — на его экране это выглядело как «ничего не
         * происходит». Убывание по десяткам даёт ранним страницам позднего
         * комплекта обойти хвост раннего: оба показывают движение. Общее время
         * не меняется — пропускная способность общая; меняется наблюдаемость.
         *
         * Нижняя граница держит хвост длинного комплекта выше фоновых задач
         * (кэш превью — 50, уборка — 10): это по-прежнему работа, которую ждёт
         * человек.
         */
        priority: Math.max(60, 100 - Math.floor(page.workingPageIndex / 10)),
      });
    }
