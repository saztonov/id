...
export function createChecksRunHandler(deps: ChecksDeps): JobHandler<'checks.run'> {
  return async (ctx: JobContext<'checks.run'>) => {
    ...
    if (snapshot === null) {
      /**
       * Отказ, а НЕ пропуск со сводкой — и это осознанный выбор.
       *
       * Соблазн был обратный: пропустить прогон с названной причиной и поставить
       * `checks.summarize`, как это делает `checks.llm_review`. Так нельзя по
       * двум причинам. Техническая: `checks.summarize` требует
       * `validationRunId`, а завести прогон без набора правил невозможно —
       * `validation_runs.ruleset_version_id` объявлен NOT NULL. Существенная:
       * сводка на непроверенном комплекте напечатала бы «замечаний не найдено».
       * Ложное «всё хорошо» на комплекте, который никто не проверял, — худший
       * из возможных исходов для портала, вся работа которого в проверке.
       *
       * Чтобы отказ перестал быть тихим, причина уходит СОБЫТИЕМ: плашка
       * конвейера читает `lastErrorMessage` мёртвой задачи и печатает этот текст
       * человеку, а лента ревизии сохраняет его в истории.
       *
       * После миграции 0044 состояние практически недостижимо: встроенный набор
       * засеян и активирован поставкой. Остаётся оно ровно для случая, когда
       * администратор снял активную версию сознательно.
       */
      const reason =
