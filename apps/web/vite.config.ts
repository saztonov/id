...
/**
 * Идентификатор выкатки: одно значение и в бандле, и в `version.json`.
 *
 ...
 * остаётся пустая строка — потребители трактуют её как «сборка не названа» и не
 * выдумывают значение.
 */
const BUILD_ID = process.env['APP_RELEASE'] ?? '';

/**
 * Метка выкатки рядом со сборкой — для сверки версий во вкладке.
 *
 * Вкладка, открытая до выкатки, держит прежний бандл сколько угодно долго и
 * узнать о новой сборке может только спросив раздачу. Файл кладётся сюда, а не
 * отдаётся эндпоинтом API: спрашивать надо ровно тот узел, который раздаёт
 * бандл (`id-web`), а не тот, который к нему отношения не имеет.
 *
 * Файл эмитится всегда, в том числе с пустым `buildId`: «файла нет» и «сборка
 * не названа» — разные состояния, и первое выглядело бы во вкладке сетевым
 * отказом. Пустую метку читатель отбрасывает сам (`src/app/buildVersion.ts`).
 */
...
export default defineConfig({
  plugins: [react(), pdfjsAssets(), versionFile()],
  server: {
    port: 5173,
  ...
  /**
   * Идентификатор сборки, вшитый в бандл.
   *
   * Потребителей двое: отчёты об ошибках браузера (§11, ADR-0010) и плашка
   * «доступна новая версия» — она сравнивает это значение с тем, что лежит в
   * `version.json` на раздаче. Обе стороны сравнения обязаны браться из одного
   * `BUILD_ID`, иначе вкладка предлагала бы обновиться после каждой сборки.
   */
  define: {
