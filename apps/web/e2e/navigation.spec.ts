/**
 * Навигация «объект → раздел → папка» через интерфейс (§3, §14).
 *
 * ## Зачем этот файл
 *
 ...
 * 2. Подрядчик заводит папку сам, ОДНИМ ФАЙЛОМ: раздел берётся из узла дерева,
 *    наименование — из имени файла. Отдельно проверено, что отказ на заливке
 *    байтов папку не выбрасывает: она остаётся, и экран даёт на неё ссылку.
 * 3. Исполнителя выводит сервер, а не форма: поля организации нет ни у одной
 *    роли, а у человека с ролями подрядчика и инженера организация НЕ берётся
 *    из второй роли — она выводится из карточки объекта с признаком «названо
 *    не человеком».
 *
 * Сценария сборки реестра здесь больше нет: реестры передачи сняты миграцией
 * 0058 вместе с согласованием, и обещать его шапкой значило бы описывать
 * портал, которого нет.
 *
 * Порядок последовательный: сценарии меняют состояние объекта, и параллельный
 * прогон делил бы одну базу.
 */
...
test('дерево навигации проходится настоящими маршрутами', async ({ page }) => {
  ...
  // строке. С S44 подзаголовок называет папку папкой, а не «работой»: ревизия
  // схлопнута в неё саму, и прежнее слово обещало уровень, которого нет.
  await expect(page.getByRole('heading', { level: 1, name: 'Комплект с разметкой' })).toBeVisible();
  await expect(page.getByText('Комплект с разметкой')).toHaveCount(1);
  await expect(page.getByText(/^Папка ИД · /u)).toBeVisible();
});
...
test('роли подрядчика и инженера вместе: организация берётся не из второй роли', async ({
  ...
  // Ответ заведения называет папку папкой (`createdFolderSchema`): поля `work`
  // сервер не отдаёт с S44, и утверждение о нём молча читало бы `undefined`.
  const { folder } = created.body as {
    folder: { contractorId: string; contractorAssumed: boolean };
  };
  expect(folder.contractorId).toBe(IDS.orgGeneral);
  expect(folder.contractorId).not.toBe(IDS.orgContractor);
  // Признак поднят: назвал не человек, и на экране будет надпись, а не имя.
  expect(folder.contractorAssumed).toBe(true);
});
