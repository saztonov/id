 ...
 *
 * Чего здесь НЕТ и почему: топ ошибок по `error_events` и медленные запросы
 * (§11) наружу маршрутом не выведены — они живут в `/metrics` и в таблице
 * `error_events`. Рисовать пустые панели с такими заголовками нельзя: пустая
 * панель «Топ ошибок» читается как «ошибок нет», а это утверждение, которого
 * портал сделать не может. Отсутствие названо прямым текстом на экране.
 */
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      ...
      <Alert
        type="info"
        showIcon
        message="Топ ошибок и медленные запросы здесь не показываются"
        description={
          'Дедупликация ошибок по отпечатку (`error_events`) и тайминги HTTP и SQL живут в ' +
          '`/metrics` и в таблице; отдельного маршрута API для них нет. Пустая панель с таким ' +
          'заголовком читалась бы как «ошибок нет» — утверждение, которого портал сделать не может.'
        }
      />
    </Space>
  );
