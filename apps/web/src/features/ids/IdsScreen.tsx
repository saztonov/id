 ...
 *
 * Объект виден подрядчику через его поставки (§4.1, фильтрация в SQL), поэтому
 * список объектов — это уже область видимости, а не общий справочник, и
 * отдельного фильтра здесь не нужно.
 *
    ...
    <>
      ...
      {objects.isSuccess && objects.data.items.length === 0 && scopeKind === 'contractor' && (
        <div style={{ marginBottom: 16 }}>
          ...
          >
            Подрядчик видит объект через свои поставки: связи «подрядчик закреплён за томом» в схеме
            нет вовсе, единственный признак закрепления — уже существующая поставка. Поэтому объект,
            где у вашей организации ещё нет ни одной поставки, сюда не попадает, и первую поставку
            на нём через портал завести нельзя — её заводит администратор портала. Пустой перечень
            здесь означает именно это, а не сбой загрузки.
          </ExplainedLimitation>
        </div>
      )}
