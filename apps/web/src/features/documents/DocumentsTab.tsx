  ...
  return (
    <Card size="small" title="Учёт страниц">
      ...
      {counts.unaccounted > 0 &&
        (counts.assigned === 0 && counts.unassigned === 0 ? (
          /*
            Нарезка не выполнялась вовсе — это НЕ нарушение инварианта.
            Различие видно прямо здесь: если ни одна страница не привязана и ни
            одна не помечена непривязанной, то документов не строили, а не
            потеряли страницы. Прежний текст в этом состоянии говорил
            «неучтённые страницы означают потерю» на каждой странице комплекта
            сразу после неудавшегося прогона — то есть пугал тем, чего не
            происходило, и уводил от настоящей причины.
          */
          <Alert
            type="warning"
            showIcon
            style={{ marginTop: 8 }}
            data-testid="pages-not-assembled"
            message="Документы ещё не собраны"
            description={
              'Ни одна страница не привязана к документу и ни одна не помечена непривязанной: ' +
              'нарезка по этой ревизии не выполнялась. Нажмите «Собрать документы»; если конвейер ' +
              'остановился отказом, причина названа в шапке ревизии.'
            }
          />
        ) : (
          <Alert
