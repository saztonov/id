 ...
 * проверенная связь, и эта выдача её не проверяет.
 *
 * Живая проба заведена отдельной ручкой (`POST /api/v1/admin/integrations/
 * rdweb-exec/check`), а не подмешана сюда, намеренно. Во-первых, открытие
 * экрана настроек не должно дёргать чужой сервис. Во-вторых, ответ пробы
 * протухает: связь, работавшая при загрузке страницы, ничего не говорит о
 * связи сейчас — удостоверение на той стороне отзывают между двумя нажатиями.
 * Поле, которое утверждало бы обратное, было бы хуже пустого места.
 */
