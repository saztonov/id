...
export const SETTINGS_REGISTRY = {
   ...
   * двенадцати крупных листах эталонного комплекта: он стоит ЗАГОЛОВКОМ ВВЕРХУ
   * листа. `near_stamp` оставляет на крупном листе, кроме штампа, один самый
   * верхний текстовый кандидат детектора: его текст доедет до страницы, и номер
   * найдёт то же правило «№ …», которым портал уже пользуется. `off` — только
   * штамп, номер обводится вручную.
   */
