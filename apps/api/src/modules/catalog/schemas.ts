...
// Категорий материалов в схемах каталога больше нет (S59): профиль раздела и
// наложения объекта их не принимают. Перечень `MATERIAL_CATEGORIES` остаётся в
// @id/contracts — его читает вывод материалов в движке правил.
 ...
 *
 * Полей `materialCategories` и `materialMatrix` в теле больше нет (S59), а
 * схема нестрогая намеренно: клиент прежней сборки, всё ещё присылающий их,
 * получает профиль без категорий, а не 422 — отказывать в настройке раздела
 * из-за поля, которое портал перестал читать, было бы наказанием за дату
 * обновления.
 *
