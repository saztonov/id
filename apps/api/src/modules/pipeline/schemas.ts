...
/**
 * Ответ кнопки «3. Проверить» (S55).
 *
 * Стадия названа `analysis`, а не `checks`, и это не описка: нажатие
 * перезапускает ВЕСЬ разбор по готовому тексту — классификацию страниц,
 * сегментацию, извлечение реквизитов, опись, граф, — а прогон правил приезжает
 * хвостом цепочки. Сказать «checks» значило бы обещать человеку секунды там,
 * где работы на минуты.
 */
export const recheckResponseSchema = z.object({
  ...
  jobCreated: z.boolean(),
  /**
   * Сколько задач прошлого разбора снято этим нажатием.
   *
   * Числом, а не молчанием: мёртвая `checks.run` держит свой `dedupe_key`
   * (0039), и пока её не снять, автопостановка проверки хвостом `graph.build`
   * тихо сливается с мертвецом. «Снято 3» отличает разбор поверх завала от
   * разбора на чистой очереди.
   */
  cancelledJobs: z.number().int().nonnegative(),
  /** Сколько прошлых прогонов правил стёрто. Ноль — проверок ещё не было. */
  clearedRuns: z.number().int().nonnegative(),
});
