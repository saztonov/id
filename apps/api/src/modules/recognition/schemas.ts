 ...
 *
 * `frozenLayoutId` обязателен и явен (§14). «Возьми текущую замороженную» было
 * бы удобнее и опаснее: между открытием экрана и нажатием кнопки инженер мог
 * заморозить следующую ревизию, и распозналась бы не та разметка, которую
 * пользователь видел.
 */
export const recognizeRequestSchema = z.object({
  frozenLayoutId: z.uuid(),
});
