 ...
 * И ставит первую задачу цепочки, а тест проверяет именно таблицу `jobs`.
 *
 * ## Почему `layoutId` обязателен и явен
 *
 * §14 требует явного идентификатора разметки, и это не формальность: «возьми
 * текущую» распознало бы не то, что видел человек, если разметку между
 * открытием экрана и нажатием кнопки пересобрали, — а узналось бы это только по
 * несовпадению координат в результатах.
 *
 * ## `Idempotency-Key`
 *
