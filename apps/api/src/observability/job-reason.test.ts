...
describe('свои классы называют причину числом', () => {
  ...
  it('отказ состояния проверок отдаёт свой текст целиком: коды правил — не числа', () => {
    // На бою человек читал «нет реализации для кодов: REG.<n>, REG.<n>» —
    // отпечаток вычеркнул ровно то, по чему видно, какой сборке правил не
    // хватает. Текст собирает сам портал из своих кодов, чужой строки в нём нет.
    const message =
      'Прогон проверок невозможен: реестр правил разошёлся с реализациями — ' +
      'нет реализации для кодов: REG.113, REG.114. Эту задачу исполняет воркер сборки, ' +
      'в которой этих правил ещё нет.';
    expect(readableJobReason({ name: 'ChecksStateError', message })).toBe(message);
    // Класс без текста — причины нет, выдумывать её нельзя.
    expect(readableJobReason({ name: 'ChecksStateError', message: '' })).toBeNull();
    expect(readableJobReason({ name: 'ChecksStateError' })).toBeNull();
  });
});
