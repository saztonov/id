...
export const PERMISSIONS = {
  ...
  /**
   * Удаление комплекта, его ревизии и черновика реестра — все пять ролей (S37).
   *
   * До этого действие было закрыто `settings.manage`, то есть администратором:
   * «стереть чужую работу вместе с историей проверок тяжелее, чем завести
   * свою». Заказчик посылку снял — удаляют все, и это решение о ПРАВЕ, а не об
   * ослаблении защиты.
   *
   * Ослабления и нет: тяжёлые случаи закрыты не правом, а помехами, и они не
   * тронуты ни одной строкой. Согласованная ревизия, переданная папка и
   * неснятый юридический запрет по-прежнему отвергают удаление у любой роли,
   * включая администратора, — потому что это уже ушло наружу и перестало быть
   * внутренним делом портала.
   *
   * Что право действительно делает шире: подрядчик может убрать СВОЙ черновик,
   * не дожидаясь администратора. Чужой он не тронет — его не видит область
   * видимости, и ответ будет 404, как на несуществующий.
   */
  ...
  /**
   * Правка разметки и отправка на распознавание.
   *
   ...
   * рисуется всегда, а `editable = can('markup.edit')` гасил в ней ВСЁ разом —
   * выбор инструмента, тип блока, повтор детекции, замену страницы. Отличить
   * «нет права» от «кнопки не работают» человеку было нечем.
   *
