...
describe('роли страниц', () => {
  ...
  it('чистит ссылку от знака номера, прочитанного буквами (S50)', () => {
    // OCR читает «№» как «Но»/«На»/«Не». В ссылку это уезжало целиком, и
    // родитель по ней не находился: на его листе напечатан только номер.
    const result = only(
      page('p1', 'ПРИЛОЖЕНИЕ\nК СЕРТИФИКАТУ СООТВЕТСТВИЯ\nНо РОСС RU.32311.0С01.ПБ01.0539'),
    );
    expect(result.parentRef).toBe('РОСС RU.32311.0С01.ПБ01.0539');
  });

  it('дата и учётный номер бланка в ссылку не входят', () => {
    // «RU.77…08.12 03.08.2012 г.» — номер родителя кончился раньше даты;
    // хвостовые цифры — учётный номер САМОГО приложения, а не родителя.
    const withDate = only(
      page(
        'p1',
        'ПРИЛОЖЕНИЕ\nК СВИДЕТЕЛЬСТВУ О ГОСУДАРСТВЕННОЙ РЕГИСТРАЦИИ\nRU.77.01.34.008.E.006609.08.12 03.08.2012 г.',
      ),
    );
    expect(withDate.parentRef).toBe('RU.77.01.34.008.E.006609.08.12');

    const withBlank = only(
      page('p1', 'ПРИЛОЖЕНИЕ\nК СЕРТИФИКАТУ СООТВЕТСТВИЯ\nРОСС RU.32311.0С01.ПБ01.0539 000345'),
    );
    expect(withBlank.parentRef).toBe('РОСС RU.32311.0С01.ПБ01.0539');
  });
