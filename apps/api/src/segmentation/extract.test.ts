...
describe('типо-специфичная схема — только при уверенно определённом типе', () => {
  ...
  it('единица привязки остаётся в номере акта: «48-ОТ/-1 этаж»', () => {
    // Опись папки «ИД Мастер апрель 2026» печатает номер целиком, а в акте
    // строчное «этаж» отбрасывалось: номером становилось «48-ОТ/-1», и все
    // двенадцать строк описи сходились с актами частичным вхождением.
    const act =
      '**АКТ**\n**освидетельствования скрытых работ** №48-ОТ/-1 этаж "10" апреля 2026г.\n' +
      '(дата составления акта)';
    const fields = extractFields(input(act, { docTypeCode: 'aosr', typeConfident: true }));

    expect(valueOf(fields, 'number')).toMatchObject({ valueText: '48-ОТ/-1 этаж' });
  });

  it('служебное слово после номера в него по-прежнему не входит', () => {
    // Чувствительность к предыдущему: список единиц закрыт, и «от», «с», «по»
    // заканчивают номер, как заканчивали.
    const passport = 'Паспорт качества № 1067 от 28.08.2025 г. выдан ООО "Тест"';
    const fields = extractFields(
      input(passport, { docTypeCode: 'quality_passport', typeConfident: true }),
    );

    expect(valueOf(fields, 'number')).toMatchObject({ valueText: '1067' });
  });
...
describe('таблица показателей паспорта качества', () => {
  // Форма паспорта КНАУФ из папки «ИД Мастер апрель 2026»: таблица реквизитов
  // бланка и таблица испытаний склеены OCR в один блок, шапка испытаний стоит
  // строкой данных внутри него.
  const PASSPORT = [
    '| | | |',
    '|---|---|---|',
    '| Паспорт № | 112 | Смесь сухая шпатлевочная |',
    '| Дата выдачи документа | 01.11.2025 | |',
    '| Результаты испытаний: | | |',
    '| Наименование показателя | Нормативное значение | Фактическое значение |',
    '|---|---|---|',
    '| 5. Начало схватывания, мин | не ранее 90 | 158 |',
    '| 7. Водоудерживающая способность, % | не менее 98 | 97,7 |',
  ].join('\n');

  it('строки таблицы уходят в реквизит, который читает правило', () => {
    const fields = extractFields(
      input(PASSPORT, { docTypeCode: 'quality_passport', typeConfident: true }),
    );

    expect(valueOf(fields, 'indicators')).toMatchObject({
      valueJson: [
        { indicator: '5. Начало схватывания, мин', norm: 'не ранее 90', fact: '158' },
        { indicator: '7. Водоудерживающая способность, %', norm: 'не менее 98', fact: '97,7' },
      ],
    });
  });

  it('таблица без графы нормы не берётся: сравнивать не с чем', () => {
    // Паспорт «КНАУФ-Тифенгрунд» той же папки печатает только факт. Придумать
    // норму здесь значило бы нарушить §8.1.
    const withoutNorm = [
      '| Наименование показателя | Фактическое значение |',
      '|---|---|',
      '| 2. Массовая доля нелетучих веществ, % | 9,1 |',
    ].join('\n');
    const fields = extractFields(
      input(withoutNorm, { docTypeCode: 'quality_passport', typeConfident: true }),
    );

    expect(valueOf(fields, 'indicators')).toBeUndefined();
  });
});
...
describe('дата выдачи не берётся у чужого документа', () => {
  ...
  it('жирная дата под номером свидетельства сильнее даты заявления и решения ТС', () => {
    // Свидетельство о государственной регистрации папки «ИД Мастер апрель
    // 2026»: своя дата напечатана «**от 06.12.2016 г.**», и звёздочка
    // закрывала находку. Датой выдачи становилась дата заявления, а у части
    // копий — дата решения Комиссии таможенного союза 2010 года, и правило
    // возраста документа сообщало «выдан 5800 дней назад».
    const fields = extractFields(
      input(
        'СВИДЕТЕЛЬСТВО\nо государственной регистрации\n' +
          '**№ RU.77.01.34.015.Е.002917.12.16**\n**от 06.12.2016 г.**\n\n' +
          'Продукция соответствует Единым требованиям, утв. решением Комиссии ' +
          'таможенного союза № 299 от 28.05.2010г.\n\n' +
          '**Заявление № 03048 от 28.11.2016 г.**',
        { docTypeCode: 'state_registration_certificate', typeConfident: true },
      ),
    );

    expect(valueOf(fields, 'issued_at')).toMatchObject({ valueDate: '2016-12-06' });
  });

  it('дата свидетельства органа по сертификации не становится датой сертификата', () => {
    // Сертификат «СМИК» той же папки: собственной даты выдачи бланк не
    // печатает вовсе, а все «от …» на листе принадлежат чужим документам.
    // Датой выдачи становилось 13.10.2020 — дата свидетельства органа.
    const fields = extractFields(
      input(
        'СЕРТИФИКАТ СООТВЕТСТВИЯ\n**RU.СМИК.001.Н.00270**\n\n' +
          'действителен с 17.05.2023 г. по 16.05.2026 г.\n\n' +
          '**ОРГАН ПО СЕРТИФИКАЦИИ** «СТМ» Свидетельство № RU.СМИК.ОС.001 от 13.10.2020 г.\n' +
          'Регистрационный номер РОСС RU.32316.04МТЛО от 13.10.2020 г.',
        { docTypeCode: 'cert_conformity', typeConfident: true },
      ),
    );

    expect(valueOf(fields, 'issued_at')).toMatchObject({ valueDate: '2023-05-17' });
    expect(valueOf(fields, 'valid_to')).toMatchObject({ valueDate: '2026-05-16' });
  });
...
describe('владелец номера и номер под заголовком (S53)', () => {
  const numberOf = (docTypeCode: string, text: string): string | null =>
    extractFields({
      docTypeCode,
      typeConfident: true,
  ...
  it('номер сертификата берётся со строки под заголовком, а не с бланка', () => {
    // Бланк сертификата так и устроен: собственный номер напечатан отдельной
    // строкой БЕЗ «№», а знак «№» стоит только у номера бланка. Двенадцать
    // копий сертификата в папке «ИД Мастер апрель 2026» получали номером бланк.
    const page = [
  ...
  it('регистрационный номер системы сертификации своим не считается', () => {
    const page = [
      ...
      '**регистрационный № РОСС RU.32311.04ТМРО**',
      '',
      '**приложение',
    ...
    expect(numberOf('cert_conformity', page)).toContain('РОСС RU.32311.ОС01.ПБ01.0539');
  });

  it('номер схемы читается сквозь разметку выделения, а не подменяется знаком «№»', () => {
    // На листе схемы значение выделено звёздочками: захват спотыкался о «*», и
    // номером становился сам знак «№».
    const page = [
      'Исполнительная схема № *48.1-ОТ/1-1 ЭТАЖ*',
      'Приложение к акту № *48-ОТ/1-1 ЭТАЖ*',
    ].join('\n');

    expect(numberOf('exec_scheme', page)).toBe('48.1-ОТ/1-1 ЭТАЖ');
  });

  it('свой номер под собственным заголовком чужим не объявляется', () => {
    // «Свидетельство» в начале строки называет ЭТОТ лист, а не чужой документ.
    const page = [
      'СВИДЕТЕЛЬСТВО',
      'о государственной регистрации',
      '№ RU.77.01.34.015.Е.002917.12.16',
...
describe('собственное название документа как ярлык номера (S53)', () => {
  const numberOf = (docTypeCode: string, text: string): string | null =>
    extractFields({
      docTypeCode,
      typeConfident: true,
  ...
  const PASSPORT = [
    ...
    '| | | |',
    '|---|---|---|',
    '| Паспорт № | 357 | грунтовка укрепляющая глубокого проникновения |',
    '| Партия № | 58071 | |',
    '',
    'Сертификат соответствия № RU.CMIK.001.H.00270 от 17.05.2023 г.',
  ].join('\n');

  it('номер берётся из графы своего бланка, а не у упомянутого сертификата', () => {
    // Знак «№» и значение стоят в РАЗНЫХ ячейках таблицы, и ни одна ступень
    // такую пару не видела: паспорту доставался номер сертификата, а строка
    // описи «Паспорт № 357» своего документа не находила.
    expect(numberOf('quality_passport', PASSPORT)).toBe('357');
  });

  it('чужое название выше своего номера его не перебивает', () => {
    // Порядок на листе обратный: ссылка на сертификат напечатана ВЫШЕ графы
    // с собственным номером. Без проверки вида ярлыком стало бы первое
    // подходящее название, и паспорт снова получил бы номер сертификата.
    const inverted = [
      'Сертификат соответствия № RU.CMIK.001.H.00270 от 17.05.2023 г.',
      '',
      '| | | |',
    ...
    expect(numberOf('quality_passport', inverted)).toBe('357');
  });

  it('заголовок чужого вида номер листу не отдаёт', () => {
    // Тот же лист, но вид не определён: упоминание сертификата всё равно не
    // становится заголовком паспорта — оно просто остаётся обычной находкой.
    const mention = ['Сертификат соответствия № RU.CMIK.001.H.00270 от 17.05.2023 г.'].join('\n');

    expect(numberOf('quality_passport', mention)).toBe('RU.CMIK.001.H.00270');
    expect(numberOf('cert_conformity', mention)).toBe('RU.CMIK.001.H.00270');
  });
});
