...
export const CANDIDATE_BASES = [
  ...
  'annex_pages',
  /**
   * Документ назвал себя номером ПАРТИИ (S57).
   *
   * У документа о качестве и паспорта своего номера часто нет вовсе, и опись
   * пишет в графе номера партию. Прежде такие решения приходили с основанием
   * `doc_no`, то есть отчёт объяснял их номером документа, которого в бумаге не
   * напечатано.
   */
  'batch_no',
  /**
   * Номера нет ни в строке, ни в документе — сошлись наименование и дата (S57).
   *
   * Основание слабее номерных по построению, и названо оно именно поэтому:
   * проверяющему видно, что решение принято без номера.
   */
  'name_and_date',
] as const;
...
export const MATCH_STATES = [
  ...
  'candidate',
  /**
   * Строка не сопоставлена, и портал НЕ утверждает, что документа нет (S57).
   *
   * «Документа нет в папке» и «портал не смог найти документ» — разные
   * утверждения, а состояние до сих пор было одно. `REG.110` отвечает на
   * `missing` замечанием «в комплекте не найден документ», отчёт печатает «нет в
   * комплекте», и оба обвиняют бумагу. Между тем строка остаётся ненайденной и
   * когда выборка кандидатов оказалась не той (документ лежит в чужом разделе
   * описи), и когда данных для решения не хватило, и когда модель недоступна.
   *
...
function basisLabel(basis: CandidateBasis): string {
  switch (basis) {
    ...
    case 'batch_no':
      return 'номером партии';
    case 'name_and_date':
      return 'наименованием и датой';
  }
}
