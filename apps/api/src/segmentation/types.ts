...
export interface ParsedRegistryRow {
  ...
  /**
   * Комплект, к которому строка относится.
   *
   * У реестра приложений комплект один — тот же, что у самого реестра, — и
   * поле остаётся пустым: его проставляет запись строк. Опись передачи, в
   * отличие от него, перечисляет всю папку и делится на разделы по актам,
   * поэтому комплект здесь несёт КАЖДАЯ строка: без него двенадцать одинаковых
   * «Паспорт № 357» сопоставлялись бы со всей папкой сразу и все двенадцать
