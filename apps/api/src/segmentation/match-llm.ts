 ...
 *
 * ## Что молчание модели НЕ отменяет
 *
 * Всё, что предфильтр УСТАНОВИЛ. Совпадение номера — факт; коллизия, когда один
 * номер сошёлся сразу у нескольких документов, — тоже факт, и притом
 * содержательный: он говорит, что различить их сравнением номеров нельзя.
 * Стереть его в «портал не сопоставил» значило бы выбросить работу, которая
 * сделана и верна, — и сделать отчёт беднее ровно там, где модель промолчала.
 *
 * ## Что молчание оставляет неизвестным
 *
 * Только `missing`. После снятия нижних ступеней лестницы «предфильтр не нашёл»
 * означает единственное: номер не совпал ни точно, ни после свёртки написания.
 * Про наличие документа в папке это не говорит ничего — у приложения своего
 * номера нет вовсе, у паспорта его заменяет партия. Утверждать по такому
 * основанию отсутствие нельзя, и состояние честно становится `undetermined`.
 */
function silent(row: PrefilterRow, why: string): MergedRow {
  if (row.matchState !== 'missing') {
    return {
      rowId: row.rowId,
