...
/**
 * Формы, снятые с корпуса `temp/MD/new` (семь настоящих комплектов).
 *
 * Корпус в репозиторий не входит (§13), поэтому здесь воспроизведены его
 * ФОРМЫ, а не данные. Каждая из трёх ниже давала на замере ноль отображений и
 * молча съедала реквизит: значение отбрасывалось как «выдуманная цитата», и
 * отличить это от «модель ничего не нашла» было нечем.
 */
describe('формы цитат из корпуса', () => {
  it('строка таблицы с пустой последней ячейкой отображается целиком', () => {
    // Проекция страницы снимала ХВОСТОВОЙ ПРОГОН разделителей, а нормализация
    // цитаты — ровно одну трубу. 61 строка из 3281 не находилась.
    const text = '| Партия № | 58071 |  |\n| Дата изготовления | 21.04.2025 |  |';
    expect(locateQuote(page(text), '| Партия № | 58071 |  |')).not.toBeNull();
  });

  it('цитата через границу ячеек отображается: в бумаге это одна строка', () => {
    // 1154 кандидата из 1154 не отображались. Модель цитирует пару
    // «ярлык — значение» слитно, потому что в бумаге они и напечатаны рядом.
    const text = '| Партия № | 58071 |  |';
    const evidence = locateQuote(page(text), 'Партия № 58071');
    expect(evidence).not.toBeNull();
    const ev = evidence as NonNullable<typeof evidence>;
    // Записывается срез ИСХОДНОГО текста — с трубой внутри, а не строка модели.
    expect(text.slice(ev.charStart, ev.charEnd)).toBe(ev.quote);
    expect(ev.quote).toBe('Партия № | 58071');
  });

  it('значение, разорванное пояснением бланка, склейкой не отображается', () => {
    // Бланк акта чередует значение и типографское пояснение в скобках. Склейка
    // кусков через пояснение — не то, что напечатано, и приниматься не должна:
    // промт поэтому требует цитировать непрерывный кусок.
    const text = [
      'ООО «Подрядчик». ОГРН 1000000000000 , ИНН 0000000000 ,',
      '(фамилия, имя, отчество (последнее - при наличии), адрес места жительства)',
      'оф.806, +7(495)000-00-00 , СРО Ассоциация «Строители»',
    ].join('\n');
    expect(locateQuote(page(text), 'ООО «Подрядчик». ОГРН 1000000000000')).not.toBeNull();
    expect(locateQuote(page(text), 'ИНН 0000000000 , оф.806')).toBeNull();
  });
});
...
describe('разбор ответа модели', () => {
  ...
  it('снятый вид (S59) — не по схеме, как и выдуманный код', async () => {
    // Код в каталоге есть, но модели он не предлагался: ответ им — ответ не из
    // списка, и принять его значило бы вернуть снятый вид через модель.
    const outcome = await classifyPageWithLlm(
      page('ЗАКЛЮЧЕНИЕ № 02(а)-2020'),
      NO_NEIGHBOURS,
      answering(answer({ doc_type: 'technical_conclusion' })),
      PROMPT,
    );
    expect(outcome.classification).toBeNull();
    expect(outcome.problem).toContain('не соответствует схеме');
