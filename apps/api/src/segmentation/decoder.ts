...
// ── Структурный признак продолжения таблицы ───────────────────────────────
 ...
 */
function continuesTable(previousText: string, text: string): boolean {
...
export function decodeSegmentation(
  ...
  for (let i = 0; i < pages.length; i += 1) {
    ...
    if (c.label === 'A-ROLE') {
      ...
      if (role === BLANK_ROLE) {
        /**
         * Пустой лист ВНУТРИ документа — его лист, а не ничей.
         *
        ...
        if (!previousInCurrent) {
          drop(page, 'пустая страница вне документа: разделитель, привязка не подтверждена');
          continue;
        }
        attach(current, page, c);
        previousInCurrent = true;
      ...
      if (AUTO_ATTACH.get(role) !== true) {
        // `signature_visual` — прямой урок корпуса: штамп ЭП печатается в
        // подвале содержательных листов, и автоприсоединение таких страниц
        // теряло бы документы целиком (CORPUS_FINDINGS, причина 5).
