 ...
 * рубеж (`UNIQUE(folder_id, source_page_id)` в `document_pages` и
...
/**
 * Сравнимая форма номера — та же, что у сверки реестра (`normalizeDocNo`).
 *
 * Своя реализация здесь была третьей по счёту и молча расходилась с общей: она
 * не знала ни гомоглифов (`ОС01` против `0С01`), ни приставки «Не», которую
 * OCR ставит вместо «№», ни хвостовой точки. Расхождение выглядело как
 * оторванное приложение: пять листов из шести в папке «ИД Мастер апрель 2026»
 * ссылались на свой сертификат нулём и не находили его.
 *
 * Довод тот же, по которому `normalizeDocNo` живёт в контрактах, а не в
 * разборе реестра: разойдись копии — часть документов молча перестанет
 * собираться, и выглядеть это будет как неполный комплект, а не как дефект
 * кода.
 */
function foldNumber(value: string): string {
  return normalizeDocNo(value).folded;
}
