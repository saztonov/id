...
// =====================================================================
// Нумерация позиций против положения строки (S53)
// =====================================================================

/**
 * Опись, в которой третий раздел пронумерован номерами второго.
 *
 * Форма снята с боевой папки «ИД Мастер апрель 2026»: раздел 10 начинается
 * баннером и строками «10.1»…«10.4», а дальше подрядчик продолжил нумерацию
 * второго десятка номерами «8.5»…«8.7». Портал возвращал такие строки в
 * ЗАКРЫТЫЙ раздел 8 — и десять строк раздела 10 сверялись с комплектом
 * восьмого акта, попутно объявляя пять документов десятого «не названными
 * описью».
 */
const MISNUMBERED_PAGE = [
  FORM_B_HEADER,
  '|---|---|---|---|---|---|---|---|',
  '| Работа восьмая (ООО "БАУТРАНС-Тест") |  |  |  |  |  |  |  |',
  '| 8 | 8.1 | АОСР Работа восьмая | №08-РР | ООО "Баутранс-Тест" | от 02.08.2024 | 1 | 1 |',
  '|  | 8.2 | Паспорт восьмой | №П-8 | ООО "Тест" | от 03.08.2024 | 1 | 2 |',
  '| Работа десятая (ООО "БАУТРАНС-Тест") |  |  |  |  |  |  |  |',
  '| 10 | 10.1 | АОСР Работа десятая | №10-РР | ООО "Баутранс-Тест" | от 04.08.2024 | 1 | 3 |',
  '|  | 10.2 | Паспорт десятый | №П-10 | ООО "Тест" | от 05.08.2024 | 1 | 4 |',
  '|  | 8.5 | Сертификат десятого | №С-10 | ООО "Тест" | от 06.08.2024 | 1 | 5 |',
  '|  | 8.6 | Декларация десятого | №Д-10 | ООО "Тест" | от 07.08.2024 | 1 | 6 |',
].join('\n');

const misnumbered = (): ReturnType<typeof parseTransferRegistry> =>
  parseTransferRegistry({ pages: [page('m-1', MISNUMBERED_PAGE)] });

describe('parseTransferRegistry: нумерация не возвращает строку в закрытый раздел', () => {
  it('строки с чужим префиксом остаются в разделе, под заголовком которого напечатаны', () => {
    const result = misnumbered();
    const tenth = result.groups.findIndex((group) => group.titleRaw === 'Работа десятая');

    expect(tenth).toBeGreaterThanOrEqual(0);
    const misplaced = result.rows.filter((row) => row.rowNo === '8.5' || row.rowNo === '8.6');
    expect(misplaced).toHaveLength(2);
    expect(misplaced.map((row) => row.groupOrdinal)).toEqual([tenth, tenth]);
  });

  it('расхождение нумерации помечено у строки, а не спрятано в предупреждении', () => {
    const rows = misnumbered().rows;

    expect(rows.find((row) => row.rowNo === '8.5')?.positionMismatch).toBe(true);
    expect(rows.find((row) => row.rowNo === '10.2')?.positionMismatch).toBe(false);
  });

  it('разделов ровно два: чужой префикс третьего раздела не заводит', () => {
    expect(misnumbered().groups).toHaveLength(2);
  });
});
