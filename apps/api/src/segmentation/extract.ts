...
/**
 * Контекст пункта 5 бланка: «Работы выполнены в период с … по …».
 *
 * Формулировка бланка РД-11-02 — «Работы выполнялись в период с … по …»; в
 * реальных актах встречаются «выполнены», «производились», «начаты/окончены».
 * Общий признак — слово о выполнении работ и следом предлог `с`/`по` с датой,
 * поэтому контекст задан широко по глаголу и узко по расстоянию: `[^\n]{0,60}`
 * не даёт совпасть с датой из соседнего пункта.
 */
const WORK_PERIOD_CONTEXT = String.raw`работ[а-я]*\s+(?:выполн|производ|начат|веден)[а-я]*[^\n]{0,60}?`;

const WORK_PERIOD_START = new RegExp(
  String.raw`${WORK_PERIOD_CONTEXT}\sс\s*[«"']?\s*(${ANY_DATE})`,
  'gidu',
);

const WORK_PERIOD_END = new RegExp(
  String.raw`${WORK_PERIOD_CONTEXT}\sпо\s*[«"']?\s*(${ANY_DATE})`,
  'gidu',
);
