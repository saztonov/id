...
/**
 * Слова, которыми открывается заголовок документа.
 *
 ...
 */
const DOCUMENT_OPENERS: readonly string[] = [
  'АКТ',
  'ПРОТОКОЛ',
  'ПАСПОРТ',
  'СЕРТИФИКАТ',
...
const OPENER_RE = new RegExp(`^(?:${DOCUMENT_OPENERS.join('|')})(?:$|[^\\p{L}])`, 'u');

/** Заголовок ищется только в самом верху: ниже начинается содержание. */
const UNKNOWN_HEADING_LINES = 3;
...
function classifyOne(
  ...
  // 7. Страница, отданная одним `image`-блоком: текста нет, решать нечем.
  ...
  // 8. Открытый мир: заголовок опознан как заголовок ДОКУМЕНТА, но ни один
  ...
  // 9. Сигнала нет. «Лист 1 из M» и «всего на N страницах» самостоятельного
