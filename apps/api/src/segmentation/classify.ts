...
import { continuesTable, opensWithTable } from './table-flow.js';
...
export const PHASE1_CONFIDENCE = {
  ...
  /**
   * Лист открывается таблицей, продолжающей таблицу предыдущего листа.
   *
   * Та же величина, что у продолжения бланка акта, и по той же причине: вывод
   * сделан из соседства двух листов, а не из утверждения на самом листе.
   * Заголовка на таком листе нет по построению — иначе таблица не была бы
   * первой строкой, — поэтому вид документа признак не называет вовсе.
   */
  tableContinuation: 0.65,
  /**
...
/**
 * «Стр.» и «Ст.» — та же запись счётчика, и без них она не читалась.
 *
 * Печатная форма сокращает слово чаще, чем пишет целиком: опись передачи
 * подписана «Стр. 1 из 4», второй лист акта — «Ст. 1 из 2». Для портала такой
 * лист был началом нового документа: четырёхстраничная опись распадалась на
 * четыре документа, а второй лист акта открывал пятый.
 */
const SHEET_COUNTER = /^(?:лист|страниц[аы]|стр\.?|ст\.)\s+(\d{1,3})\s+из\s+(\d{1,3})\b/iu;

/**
 * Счётчик ПЕРВОГО листа, где бы он на строке ни стоял.
 *
 * Один скан-лист несёт не всегда один лист документа. Сертификат калибровки
 * `№240545` комплекта `№01_Бл_П` отснят разворотом: на одной странице подряд
 * идут «Дата калибровки: 15.04.2024 г. Страница 1 из 2» и — ниже, отдельной
 * строкой — «Страница 2 из 2». `SHEET_COUNTER` привязан к началу строки
 * намеренно (иначе «Лист 2 приложения №5» стал бы счётчиком) и первую запись не
 * видит: перед ней стоит дата. Вторую видит — и лист, НАЧИНАЮЩИЙ документ,
 * объявлялся его продолжением.
 *
 * Цена ошибки здесь максимальная из возможных: документ не заводится вовсе, его
 * строка реестра получает «нет в комплекте», и портал обвиняет комплект в
 * отсутствии листа, который в нём лежит.
 *
 * Поэтому вопрос «продолжение ли это» решается не первой найденной записью, а
 * наличием ПЕРВОГО листа: страница, на которой напечатано «лист 1 из M»,
 * продолжением не является ни при каких других счётчиках на ней. Привязки к
 * началу строки здесь нет и не нужно — «1 из M» неоднозначным не бывает, а
 * форма «лист 1» без «из M» сюда не попадает вовсе.
 */
const OPENING_SHEET_COUNTER =
  /(?:^|[\s|(])(?:лист|страниц[аы]|стр\.?|ст\.)\s+1\s+из\s+(\d{1,3})\b/iu;
...
function classifyOne(
  ...
  // Первый лист документа отменяет решение о продолжении: он утверждает, что
  // документ здесь НАЧИНАЕТСЯ, а прочие счётчики на том же скане относятся к
  // его же следующим листам, снятым одним кадром. Подсказку об ожидаемом объёме
  // счётчик при этом сохраняет — она ниже, в ветке «сигнала нет».
  const opensDocument = normalized.some((line) => OPENING_SHEET_COUNTER.test(line));
  if (counter && counter.sheetNo > 1 && !opensDocument) {
  ...
  // 9.5. Лист открывается таблицей, продолжающей предыдущий: продолжение
  //    документа. Признак структурный, и другого у такого листа нет — ни
  //    заголовка, ни счётчика, ни якоря роли (их шаги выше уже не нашли).
  //
  //    Условие «заголовка не опознано» существенно: страница, где таблица идёт
  //    ПОД собственным титулом, начинает документ, а совпадение ширины граф с
  //    предыдущим листом — случайность. Отсюда и требование, чтобы таблица
  //    была ПЕРВОЙ строкой листа.
  //
  //    Без этого шага четырёхстраничная опись передачи распадалась на два
  //    документа: её вторые листы уходили в `U`, и решение доставалось
  //    LLM-фазе, которая на одном таком листе отвечала «продолжение», а на
  //    другом — «новый документ» с уверенностью 0.95. Вид документа шаг не
  //    присваивает: продолжение говорит «лист чужой», а не «какого он вида».
  if (continuesPreviousTable && opensWithTable(page.text) && unknownHeadingIndex < 0) {
    return {
      ...NO_SIGNAL,
      label: 'I-DOC',
      pageRoleCode: DOC_CONTINUATION,
      confidence: PHASE1_CONFIDENCE.tableContinuation,
      source: 'blocks',
      reason: `лист открывается таблицей, продолжающей таблицу предыдущего листа${note}`,
    };
  }
