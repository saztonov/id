...
/**
 * Потолок интервала сердцебиения.
 *
 * Он же — задержка между нажатием «Стоп» и обрывом идущей задачи (S50): отмена
 * узнаётся тем же стуком. Двадцать секунд было выбрано под единственный вопрос
 * «жива ли аренда», и для него годилось; для кнопки, которой человек
 * останавливает вызов модели, это долго — он успевает нажать второй раз. Десять
 * секунд удваивают число стуков (по одному на идущую задачу), и это дешёвый
...
/**
 * Задачу сняли, пока она выполнялась (S50).
 *
 * Отдельный класс от `LeaseLostError`, потому что вопросов два и ответы на них
 * разные: перехват аренды — гонка воркеров, о которой надо знать
 * эксплуатации; отмена — штатное действие человека, нажавшего «Стоп». Пока
 * класс был один, журнал сообщал о гонке при каждой остановке, а обработчики,
 * решающие по классу отказа судьбу прогона, не могли отличить «работу
 * прекратили» от «работа сломалась».
 */
export class JobCancelledError extends Error {
  constructor() {
    super('задача снята: обработку остановил человек');
 ...
 * ждёт терминальности страниц прогона, `rd.sync_poll` — окончания распознавания
 * на стороне RD WEB. Выразить это было нечем: единственным
...
export class JobRunner {
  ...
  /**
   * Назначить следующий проход очереди.
   *
   * Прежний таймер снимается ЯВНО, и это существенно с S50: будильников стало
   * два — итог захвата и освободившийся слот, — а два живых таймера одной
   * очереди дали бы два одновременных `#pump`. Оба увидели бы свободный слот,
   * оба захватили бы по задаче, и потолок параллелизма — тот самый, которым
   * очередь `cpu` защищает память воркера, — оказался бы вдвое выше
   * объявленного.
   */
    ...
    for (const job of claimed) {
      ...
      const promise = this.#runJob(job, controller)
        ...
        .finally(() => {
          ...
          /**
           * Освободившийся слот будит очередь сразу (S50).
           *
           * Прежде тик перепланировался только по итогу ЗАХВАТА: захватили —
           * сразу за следующей пачкой, не захватили — сон на интервал опроса.
           * Но при заполненном пуле захвата не происходит вовсе, и цикл засыпал
           * на секунду ровно тогда, когда работа кончалась. На очереди `cpu` с
           * потолком в одну задачу это давало полсекунды простоя в среднем на
           * КАЖДУЮ страницу: на 220 листах — четыре минуты из сорока, и столько
           * же на пробе ориентации.
           *
           * Планировать безопасно: `#schedule` молчит при остановке воркера, а
           * `#pump` на пустой очереди вернёт ноль и заснёт обычным порядком.
           */
      ...
      void renewLease(this.#options.db, {
        ...
        .then((renewal) => {
          ...
          if (renewal === 'cancelled') {
            logger.info(
              { event: 'job_cancelled' },
              'задача снята во время исполнения, прерываем попытку',
            );
  ...
  async #finishFailed(
    ...
    // Разбор имени ошибся бы молча на `layout.analyze_coverage` (стадия разметки,
