...
describe('смена пароля', () => {
  ...
  it('отвергает попытку оставить прежний пароль', async () => {
    // Длине `qwedcxz1@` удовлетворяет, но новый пароль не должен совпадать с
    // текущим, поэтому задать его заново через форму нельзя. Пароль первого
    // входа и не должен переживать первый вход; проверка существует, чтобы это
    // следствие не выглядело дефектом при разборе.
    const session = await login(LOGIN, PASSWORD);
    const response = await app.inject({
      method: 'POST',
