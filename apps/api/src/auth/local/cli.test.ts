...
describe('разбор аргументов', () => {
  ...
  it('отказывается принимать логин не в форме адреса', () => {
    // До подключения Keycloak логином служит адрес. Учётная запись с логином
    // `admin` была бы недоступна из браузера: форма входа не отправит значение
    // без собаки.
    expect(() => parseArguments(['create', '--email', 'admin', '--name', 'Админ'])).toThrow(
      /не похож на адрес/u,
    );
  });

  it('снимает обрамляющие пробелы адреса', () => {
    expect(parseArguments(['create', '--email', `  ${EMAIL}  `, '--name', 'Админ']).email).toBe(
      EMAIL,
    );
  });
