...
export async function buildApp(options: BuildAppOptions = {}): Promise<AppInstance> {
  ...
  // Разметка: кнопка «Разметить файл», ревизии разметки и правка блоков
  // (§6.1, §7). Маршрут, не зарегистрированный здесь, недостижим — это ровно
  // тот отказ, которым закончились S3 и S5.
