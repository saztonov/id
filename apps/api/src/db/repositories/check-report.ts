...
export function registryRowVerdict(input: {
  ...
  /** Кто решил: лестница номеров или модель (S57). */
  ...
  /**
   * Подпись строки — ДОВОД, а не перевод числа порогами (S57).
   *
   * До S57 отчёт восстанавливал смысл из счёта: 0.85 и выше — «начертание
   * номера отличается», ниже — «номер совпал не полностью». Число смысла не
   * хранит, и на боевой папке «ИД Мастер апрель 2026» сорок строк получили одну
   * подпись на четыре разных случая: приложение без своего номера, тот же номер
   * в другой раскладке, совпадение по куску и партия вместо номера.
   *
   * Теперь довод приходит от того, кто решал, и печатается как есть. Порогов
   * здесь не осталось вовсе — это и есть проверка того, что довод настоящий: их
  ...
  /**
   * «Не сопоставлено» — не «нет в комплекте» (S57).
   *
   * Строка остаётся ненайденной и когда выборка кандидатов оказалась не той, и
   * когда данных не хватило, и когда модель недоступна. Подпись обязана
   * говорить о ПОРТАЛЕ, а не о папке: иначе проверяющий идёт искать бумагу,
   * которая лежит на своём месте.
   */
  if (input.matchState === 'undetermined') {
    return {
      status: 'undetermined',
      text: note === '' ? 'портал не сопоставил строку' : `портал не сопоставил строку: ${note}`,
    };
  }
  ...
  if (input.matchState === 'ambiguous') {
    return {
      ...
      text:
        note === ''
          ? 'подошли несколько документов — какой именно, неизвестно'
          : `подошли несколько документов: ${note}`,
    };
  }

  if (input.matchState === 'candidate') {
    // Кандидат ничего не утверждает: документ не подтверждён, а похожий в
    // комплекте есть. Решает человек — и решать ему нечем, пока строка не
    // называет, ЧТО именно похоже.
    const head = note === '' ? 'документ не подтверждён; сверьте вручную' : note;
    return { status: 'warning', text: `${head}${input.candidates}` };
  }
