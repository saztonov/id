...
it('сброс конвейера сохраняет порядок DERIVED_DELETES и щадит разметку', () => {
  ...
  //
  // Реестр внешних идентификаторов RD WEB добавлен в S52 по тому же доводу и с
  // той же ценой ошибки: он отвечает на вопрос «этот блок уже объявлялся?», и
  // снесённый вместе с результатами превращал бы каждое повторное
  // распознавание в полное — RD WEB объявил бы `new_block` по всему комплекту.
  ...
  // Журнал отправок, наоборот, уходит: он ссылается на прогон, и пережить его
  // удаление не может ни физически, ни по смыслу. Счётчик генераций при этом
  // живёт в `rd_exec_documents` и сброс переживает.
  expect(reset).toContain('rd_exec_syncs');
