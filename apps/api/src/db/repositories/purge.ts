...
/**
 * Таблицы прогона правил — то, что стирается перед повторной проверкой.
 *
 * Список нужен потому, что «стереть предыдущую проверку» — это не то же самое,
 * что сбросить конвейер: распознанный текст, разметка и разобранные документы
 * обязаны пережить нажатие целиком, иначе кнопка «3. Проверить» ничем не
 * отличалась бы от «Распознать полностью».
 */
const CHECKS_RESET_TABLES: readonly string[] = ['finding_evidence', 'findings', 'validation_runs'];

/**
 * Стирание прошлой проверки — фильтром по `DERIVED_DELETES`, а не третьим списком.
 *
 * Порядок наследуется оттуда и остаётся верным: доказательства ссылаются на
 * замечания, замечания — на прогон, поэтому идут именно так. Второй экземпляр
 * этого знания разошёлся бы с первым при первой же новой таблице проверок.
 */
export const CHECKS_RESET_DELETES: readonly PurgeStep[] = DERIVED_DELETES.filter((step) =>
  CHECKS_RESET_TABLES.includes(step.table),
);

/**
 * Снести прошлые прогоны правил, не тронув ничего выше.
 *
 * ## Почему стирание явное, хотя экран и так показывает один прогон
 *
 * `resolveShownRun` отдаёт последний прогон, и с точки зрения пользователя
 * предыдущая проверка исчезает сама. Но `findings.target_id` внешнего ключа не
 * имеет — цель полиморфна по построению (0006), — а повторный разбор удаляет и
 * пересоздаёт `logical_documents`. Без стирания в базе копятся замечания,
 * указывающие на документы, которых больше нет: запрос по явному
 * `validationRunId` вернёт их как настоящие, и отличить их от живых будет нечем.
 *
 * ## Почему задачи снимает вызывающий, а не эта функция
 *
 * В отличие от `resetPipelineForFolder`, здесь снос строк уже одной стадии, а
 * снимать перед повторным разбором надо задачи ДВУХ — `analysis` и `checks`.
 * Спрятать сюда отмену половины из них значило бы разделить одно решение между
 * маршрутом и репозиторием, оставив вызывающего гадать, что уже снято.
 */
export async function resetChecksForFolder(executor: Executor, folderId: string): Promise<number> {
  // Счёт снимается ДО удаления отдельным запросом, а не `returning` внутри
  // `runDeletes`: список удалений — общий для трёх операций, и заставлять его
  // возвращать строки ради одной из них значило бы платить этим за каждую.
  const existing = await executor.execute(
    sql`select id from validation_runs where folder_id = ${folderId}::uuid`,
  );
  await runDeletes(executor, CHECKS_RESET_DELETES, folderId);
