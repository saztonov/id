 ...
 *
 * `contractorId` берётся из собственной строки пользователя и ограничивает
 * выборку. `objectId` — из назначений, и с S37 он НЕ ограничивает ничего:
 * областей по объектам не осталось. Колонка остаётся здесь только потому, что
 * `ScopeTarget` требует пары, и подставить вместо неё «что-нибудь похожее»
 * значило бы солгать о смысле поля.
 *
 * Соединение обязано быть LEFT и по прежней причине: пользователь без
 * назначений должен остаться видимым, а INNER JOIN молча выбросил бы его.
 */
