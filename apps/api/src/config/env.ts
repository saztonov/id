...
/**
 * Секрет, который уходит в заголовок HTTP дословно.
 *
 * Проверяется не форма удостоверения, а пригодность значения для заголовка:
 * пробельные символы внутри и обрамляющие кавычки — два самых частых способа
 * испортить токен по дороге из выдачи в окружение. Скопированное вместе со
 * значением слово `Bearer` даёт на проводе `Bearer Bearer …`, и удалённая
 * сторона, срезав схему, видит мусор; кавычки из env-файла уезжают в заголовок
 * вместе с токеном. Оба случая на той стороне выглядят как «удостоверение не
 * признано» — то есть неотличимо от отозванного токена, и стоят часов поиска.
 *
 * Про форму САМОГО удостоверения здесь намеренно не сказано ничего: как оно
 * выводится из выданной пары, решает удалённая сторона, и повторять её правило
 * у себя значило бы завести второе место, где оно может разойтись с первым
 * (см. докблок `RDWEB_EXEC_TOKEN`). Непригодный префикс ловит живая проба связи
 * из админки, а не старт процесса.
 */
const headerSecret = (name: string) =>
  nonPlaceholder(name)
    .refine(
      (v) => !/\s/u.test(v),
      `${name}: значение уходит в заголовок дословно и не может содержать пробельных символов — похоже, вместе с токеном скопировано слово Bearer`,
    )
    .refine(
      (v) => !/^["']|["']$/u.test(v),
      `${name}: значение обрамлено кавычками — в заголовок они уедут вместе с токеном`,
    );
...
const envSchema = z.object({
   ...
   *
   * Умолчание 8 — минимум корпоративного стандарта и единственное требование к
   * паролю: проверок сложности политика не выполняет. Развёртывание, которому
   * нужен более длинный пароль, поднимает значение здесь.
   */
  AUTH_LOCAL_PASSWORD_MIN_LENGTH: z.coerce.number().int().min(8).max(256).default(8),
  ...
  /**
   * Откуда берутся превью страниц на экране разметки.
   *
   * Значение осталось одно. Прежний режим `cached` складывал в наше
   * хранилище картинки, отрендеренные RD WEB, и держался на ручке превью
   * легаси-API — вместе с ней он и ушёл. Оставлять перечисление из одного
   * значения незачем: переключатель, у которого нет второго положения,
   * обещает выбор, которого нет.
   */
  PREVIEW_MODE: z.literal('pdfjs').default('pdfjs'),
  ...
  /*
   * Контур снимка исполнительной документации RD WEB (контракт document-sync v1).
   *
   * Адрес отдельной переменной, а не общей «базой RD WEB»: у контура своё
   * удостоверение и свой список проектов, и общая переменная означала бы, что
   * настроив соседний контур, администратор молча включает этот.
   */
  RDWEB_EXEC_BASE_URL: z.string().url().optional(),
  /**
   * Удостоверение контура: значение заголовка ПОСЛЕ слова `Bearer`.
   *
   * Кладётся готовым, а не собирается из `principal_id` и секрета: как именно
   * удостоверение выводится из выданной пары, решает RD WEB, и повторять их
   * правило у себя значило бы завести второе место, где оно может разойтись с
   * первым. Токен показывается администратору ровно один раз при выпуске и
   * живёт ТОЛЬКО здесь: ни в `app_settings` (§10), ни в журнале его быть не
   * должно, а `nonPlaceholder` заодно не даёт доехать до стенда значению из
   * `.env.example`.
   */
  RDWEB_EXEC_TOKEN: headerSecret('RDWEB_EXEC_TOKEN').optional(),
  /**
   * Проект RD WEB, в который портал шлёт снимки.
   *
   * Одно значение на развёртывание, а не карта «объект → проект»: у токена
   * явный список разрешённых проектов, и пустой список означает «ни одного», а
   * не «все». Карта появится, когда проектов станет больше одного; настройка,
   * которую сегодня нечем заполнить, была бы обещанием.
   */
  RDWEB_EXEC_PROJECT_ID: z.string().min(1).max(128).optional(),
  /** Потолок ожидания одного вызова контура. */
  RDWEB_EXEC_TIMEOUT_MS: z.coerce.number().int().positive().default(120_000),
  ...
  /**
   * Версия развёрнутого кода: попадает в каждую строку журнала ошибок и в
   * почасовой ряд.
   *
   * Без неё вопрос «сломалось ли после вчерашнего деплоя» решается сверкой
   * времени всплеска с временем выкатки по памяти — то есть не решается.
   * Умолчания нет намеренного значения: `unknown` в ряде честно говорит «эта
   * установка релиз не сообщает», тогда как подставленная версия из
   * package.json совпадала бы у всех сборок одной ветки и врала бы.
   *
   * Пустая строка равна отсутствию (S58): значение подставляет выкатка через
   * `environment` compose (`${APP_RELEASE:-}`), и запуск compose без переменной
   * — `ps`, `logs`, `exec` — передал бы пустоту, которая роняла бы старт.
   * Это же значение сверяет забор сборки воркера (`jobs/build-fence.ts`).
   */
  APP_RELEASE: z.preprocess(
    (value) => (value === '' ? undefined : value),
    z.string().min(1).max(120).optional(),
  ),
  /**
   * Как часто накопитель журнала уходит в БД.
   *
   * Две секунды — размен между потерей при падении процесса и стоимостью
   * записи. Меньше — теряется смысл пакета (при шторме запросов становится
   * снова много); больше — растёт окно, в котором события живут только в
   * памяти и исчезают вместе с процессом.
   */
  ERROR_JOURNAL_FLUSH_MS: z.coerce.number().int().min(200).max(60_000).default(2_000),
  /**
   * Как редко повторяется ОДНА И ТА ЖЕ комбинация в примерах.
   *
   * Пять минут: примеры нужны для разбора, а не для счёта — точные числа
   * живут в почасовых бакетах. Новая комбинация (релиз, маршрут, домен,
   * стадия, код) пишется сразу и этим порогом не задерживается.
   */
  ERROR_JOURNAL_SAMPLE_INTERVAL_MS: z.coerce
    .number()
    .int()
    .min(1_000)
    .max(3_600_000)
   ...
   * порождает новый отпечаток на каждое событие. Сверх лимита события
   * учитываются счётчиком служебной проблемы — «журнал захлебнулся» обязано
   * быть видно, а «журнал пуст» этого не сообщает.
   */
  ERROR_JOURNAL_NEW_SIGNATURE_LIMIT: z.coerce.number().int().min(10).max(100_000).default(500),
  /**
   * Сроки хранения слоёв журнала (§11).
   *
   * Разные, потому что слои отвечают на разные вопросы. Примеры — сырьё для
   * разбора: они несут контекст, и держать их год значит держать год
   * потенциально чувствительных подробностей ради вопросов, которые задают в
   * первые недели. Почасовой ряд отвечает «когда началось» и «стало ли лучше
   * после исправления», и вот он нужен на годовом горизонте — при этом он
   * состоит из чисел и не содержит ничего, что можно было бы утратить.
   * Карточки проблем и решения не удаляются вовсе: они и есть накопленное
   * знание, ради которого журнал держат.
   */
  ERROR_SAMPLE_RETENTION_DAYS: z.coerce.number().int().positive().default(90),
  ERROR_STATS_RETENTION_DAYS: z.coerce.number().int().positive().default(365),
  /**
   * Срок счётчиков медленных операций.
   *
   * Короче, чем у ряда ошибок: производительность интересна свежая. «Год назад
   * этот маршрут был медленным» не приводит ни к какому решению — систему с тех
   * пор меняли, и сравнивать не с чем.
   */
  SLOW_OPERATION_RETENTION_DAYS: z.coerce.number().int().positive().default(90),
  /** Потолок примеров на одну проблему: экран, а не выгрузка. */
  ERROR_SAMPLES_PER_ISSUE: z.coerce.number().int().min(10).max(10_000).default(200),

  /**
   * Приём ошибок браузера (§11).
   *
   * Включён по умолчанию: без него исключение при отрисовке гасит интерфейс в
   * белый экран и не оставляет следа нигде — пользователь видит поломку,
   * которой для портала не существует. Выключение регистрирует маршрут не
   * отказом, а его отсутствием: маршрут, которого нет, нельзя вызвать по
   * ошибке, забыв проверку флага.
   */
  CLIENT_ERROR_REPORTING: z
    .string()
    .default('true')
    .transform((v) => v !== 'false'),
  /**
   * Лимит отчётов с одного адреса в минуту.
   *
   * Тридцать — заведомо больше, чем способна прислать вкладка с исправным
   * подавлением повторов (её собственный потолок — десять на вкладку), и
   * заведомо меньше, чем нужно, чтобы наполнить журнал с одной машины.
   */
  CLIENT_ERROR_RATE_LIMIT_MAX: z.coerce.number().int().min(1).max(10_000).default(30),
  /**
   * Общий лимит приёма на процесс в минуту.
   *
   * Второй рубеж на случай распределённого источника, против которого лимит по
   * адресу бесполезен. Считается в памяти, а не в БД: рубеж существует ровно
   * для того, чтобы под потоком НЕ ходить в базу.
   */
  CLIENT_ERROR_GLOBAL_LIMIT_PER_MIN: z.coerce.number().int().min(10).max(100_000).default(600),
  METRICS_ENABLED: z
    .string()
    .default('true')
    .transform((v) => v !== 'false'),
...
function crossChecks(env: Env): string[] {
  ...
  /*
   * Контур снимка включается наличием ЛЮБОЙ из трёх переменных.
   *
   * Проверка «задана одна — обязаны быть все» строже, чем «задан адрес —
   * обязательны остальные», и разница существенна: администратор, положивший
   * токен и забывший проект, при втором правиле поднял бы портал с молча
   * выключенной интеграцией, а при этом получает внятный отказ старта. Половина
   * конфигурации хуже её отсутствия: кнопка «Распознать» ставила бы задачи, и
   * каждая падала бы на первом же вызове.
   */
  const execVars = ['RDWEB_EXEC_BASE_URL', 'RDWEB_EXEC_TOKEN', 'RDWEB_EXEC_PROJECT_ID'] as const;
  if (execVars.some((key) => Boolean(env[key]))) {
    for (const key of execVars) {
      if (!env[key]) errors.push(`${key} обязателен при заданной интеграции RDWEB_EXEC_*`);
    }
  }
