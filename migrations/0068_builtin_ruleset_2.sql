...
SELECT $rules$builtin-2$rules$, $ruleset$builtin$ruleset$,
       $ruleset$Набор по умолчанию из каталога правил портала. Опубликован поставкой, а не человеком.$ruleset$
 WHERE NOT EXISTS (SELECT 1 FROM ruleset_versions WHERE version = $rules$builtin-2$rules$);
...
SELECT v.id, x.rule_code, x.is_enabled, x.severity, x.is_blocking, x.params
 ...
 WHERE v.version = $rules$builtin-2$rules$ AND v.published_at IS NULL
...
UPDATE ruleset_versions
 ...
 WHERE version = $rules$builtin-2$rules$ AND published_at IS NULL;
...
SELECT $ruleset$ruleset.active_version_id$ruleset$, to_jsonb(id::text)
  FROM ruleset_versions WHERE version = $rules$builtin-2$rules$
